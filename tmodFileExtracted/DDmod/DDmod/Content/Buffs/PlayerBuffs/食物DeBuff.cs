            ...
            for (int A = 0; A < player.GetModPlayer<FoodPlayer>().FoodBuff.Length; A++)
            {
                ...
                if (buff != null && buff.type > 0&& buff.DeBuff)
                {
                    if (buff.type == FoodBuff.伤害)
                    {
                        ...
                        text += Damage + " -" + (Ratio * 100).ToString("F1") + "%   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.防御)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.防御") + " -" + Ratio + "   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.防御2)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.防御") + " -" + (Ratio * 100).ToString("F1") + "%   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.移速)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.移动速度") + " -" + (Ratio * 100).ToString("F1") + "%   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.生命回复)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.生命回复") + " -" + Ratio + "   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.生命)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.生命") + " -" + Ratio + "   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.生命2)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.生命") + " -" + (Ratio * 100).ToString("F1") + "%   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.魔力回复)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.魔力回复") + " -" + Ratio + "   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.魔力)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.魔力") + " -" + Ratio + "   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.魔力2)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.魔力") + " -" + (Ratio * 100).ToString("F1") + "%   " + "(" +T + ")" + "\n";
                    }
                    if (buff.type == FoodBuff.挖掘)
                    {
                        text += Language.GetTextValue("Mods.DDmod.properties.挖掘速度") + " -" + (Ratio * 100).ToString("F1") + "%   " + "(" +T + ")" + "\n";
                    }
                }
            }
            tip = text;
