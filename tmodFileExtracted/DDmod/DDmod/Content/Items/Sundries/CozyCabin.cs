...
{
    public class CozyCabin : ModItem
    {
        public Asset<Texture2D> 温馨木屋物块;
        public Asset<Texture2D> 温馨木屋墙;
        public Asset<Texture2D> 温馨木屋家具;
        public Asset<Texture2D> 温馨木屋锤;
        public override void Load()
        {
            温馨木屋物块 = ModContent.Request<Texture2D>("DDmod/Content/Items/Sundries/温馨木屋物块");
            温馨木屋墙 = ModContent.Request<Texture2D>("DDmod/Content/Items/Sundries/温馨木屋墙");
            温馨木屋家具 = ModContent.Request<Texture2D>("DDmod/Content/Items/Sundries/温馨木屋家具");
            温馨木屋锤 = ModContent.Request<Texture2D>("DDmod/Content/Items/Sundries/温馨木屋锤");
        }
        ...
        public override bool? UseItem(Player player)
        {
            ...
            {
                Point point = new Point((int)Main.MouseWorld.X / 16, (int)Main.MouseWorld.Y / 16 - 温馨木屋物块.Height()+1);
                PlaceBlock(温馨木屋物块.Value, point, player);
                PlaceWall(温馨木屋墙.Value, point, player);
                PlaceFurniture(温馨木屋家具.Value, point, player);
                PlaceHammer(温馨木屋锤.Value, point, player);

                if (Main.netMode == NetmodeID.MultiplayerClient)
                {
                    NetMessage.SendTileSquare(player.whoAmI, point.X, point.Y, 温馨木屋物块.Width(), 温馨木屋物块.Height());
                }
                Item.stack--;
            }
            return false;
        ...
        public void PlaceBlock(Texture2D texture, Point Position, Player player)
        {
            ...
            for (int i = 0; i < colors.Length; i++)
            {
                ...
                int y = Position.Y + i / texture.Height;
        ...
        //放置家具
        public int ColorHammer(Color color)
        {
            //向右斜
            if (color == new Color(255, 255, 0))
            {
        ...
        public void PlaceHammer(Texture2D texture, Point Position, Player player)
        {
            Color[] colors = DDHelper.GetColors(texture);
            for (int i = 0; i < colors.Length; i++)
            {
                int x = Position.X + i % texture.Width;
                int y = Position.Y + i / texture.Height;
                int Furniture = ColorHammer(colors[i]);
                if (Furniture >= 0)
                {
                    var tile = Main.tile[x, y];
                    tile.BlockType = (BlockType)Furniture;
                }
            }
        ...
        {
            CreateRecipe(1).AddIngredient(9, 120).AddIngredient(ItemID.Torch, 3).AddTile(TileID.WorkBenches).Register();
        }
    }
}
