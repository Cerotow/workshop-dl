            ...
            if (NPCdirection.FindClosest(player.Center, 250) != null)
            {
                ...
                if (vector.Length() < 250)
                {
                    ...
                    for (int a = 0; a < 5; a++)
                    {
                        ...
                        {
                            T = NewProjectile(player.GetSource_FromAI(), player.Center.X, player.Center.Y, 0f, 0f, ModContent.ProjectileType<ServantOfTheHeart>(), 45, 3f, player.whoAmI, Proj, T);
                            Main.projectile[T].originalDamage = 45;
                            Proj++;
                        }
                    }
                }
