...
namespace DDmod.Content.Items.Boss.流星破坏者
{
    public class 流星保险箱 : ModItem
    {
        ...
        {
            int[] A = new int[4];
            A[0] = ModContent.ItemType<流星炮>();
            A[1] = ModContent.ItemType<流星召唤杖>();
            A[2] = ModContent.ItemType<破坏者巨刃>();
            A[3] = ModContent.ItemType<破坏者激光枪>();

            itemLoot.NormalLoot(1, 1, 1, 1, A);

            itemLoot.Add(ItemDropRule.Common(ModContent.ItemType<破坏者核心装置>()));

            itemLoot.Add(ItemDropRule.CoinsBasedOnNPCValue(ModContent.NPCType<NPCs.Boss.流星破坏者.流星破坏者>()));
        }
