...
{
    public class SonOfTheLifeGuard : ModProjectile
    {
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            Texture2D eyeTexture = (Texture2D)ModContent.Request<Texture2D>("DDmod/Content/Projectiles/Pet/MasterPet/ServantOfTheSonOfLife");
            ...
            for (int i = 0; i < eyeCount; i++)
            {
                ...
                int num22 = (int)(Main.player[Projectile.owner].Center.Y / 16 - Main.worldSurface);
                Vector2 drawPos = orbitingCenter - Main.screenPosition + rotatedPos;
                if (num22 < 0)
                {
                    Main.EntitySpriteDraw(eyeTexture, drawPos, eyeTexture.Bounds, lightColor, rotatedPos.ToRotation() + MathHelper.PiOver2, TextureAssets.Projectile[Projectile.type].Size() / 2, 1f, SpriteEffects.None, 0);
                }
                else
                {
                    Vector3 vector = new Vector3(0.75f, 0.25f, 0.25f);
                    ...
                    Main.EntitySpriteDraw(eyeTexture, drawPos, eyeTexture.Bounds, new Color(55, 0, 0, 0), rotatedPos.ToRotation() + MathHelper.PiOver2, TextureAssets.Projectile[Projectile.type].Size() / 2, 1f, SpriteEffects.None, 0);
                    Main.EntitySpriteDraw(eyeTexture, drawPos, eyeTexture.Bounds, new Color(255, 0, 0, 0), rotatedPos.ToRotation() + MathHelper.PiOver2, TextureAssets.Projectile[Projectile.type].Size() / 2, 1f, SpriteEffects.None, 0);
                }
        ...
        {
            if (!player.dead && (player.HasBuff(ModContent.BuffType<SonOfTheLifeGuardBuff>()) || player.HasBuff(ModContent.BuffType<星心之子Buff>())))
            {
