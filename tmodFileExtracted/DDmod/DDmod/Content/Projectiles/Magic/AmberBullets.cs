...
{
    public class AmberBullets : ModProjectile
    {
            ...
            if (Projectile.localAI[0] == 1)
            {
                ...
                for (int a = 1; a < Projectile.scale / 2; a++)
                {
                    if (Projectile.ai[1] == 0)
                    {
                        ...
                        Projectile projectile = Main.projectile[NewProjectileChange(Projectile.GetSource_FromThis(), Projectile.Center + Position, Position * Projectile.scale / 300, Projectile.type, (int)(Projectile.damage * R / 10), Projectile.knockBack, Projectile.owner, 0, 1, Projectile.DProj().Magnification * R)];
                        projectile.scale = R;
                    }
                }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if (Projectile.localAI[0] == 0)
            {
                ...
                for (int i = 0; i < Projectile.oldPos.Length; i++)
                {
                    ...
                    Color color = new Color(255, 155, 48, 0) * ((Projectile.oldPos.Length - i) / (float)Projectile.oldPos.Length / 2f);
                ...
                Main.spriteBatch.Draw(texture, Projectile.position + vector - Main.screenPosition, null, new Color(255, 155, 48, 0), Projectile.rotation, texture.Size() / 2, Projectile.scale, spriteEffects, 0f);
                Main.spriteBatch.Draw(texture, Projectile.position + vector - Main.screenPosition, null, new Color(0, 100, 207, 0), Projectile.rotation, texture.Size() / 2, Projectile.scale / 1.5F, spriteEffects, 0f);
            }
            return false;
        }
    }
