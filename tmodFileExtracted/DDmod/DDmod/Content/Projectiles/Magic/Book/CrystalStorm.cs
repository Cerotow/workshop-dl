...
{
    public class CrystalStorm : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (Projectile.ai[0] >= player.HeldItem.useAnimation && Projectile.DProj().Bool[1])
            {
                ...
                int Type = 70;
                vector2 = Utils.RotatedBy(vector2, Main.rand.NextFloat(-0.1F, 0.1F), default);
                if (Main.rand.NextBool(6))
                {
                    for (float A = 0; A < 50; A++)
                    {
                        ...
                        dust.noGravity = true;
                        dust.scale = 1.5f;
                        dust.velocity = Main.rand.NextVector2Unit() * Main.rand.NextFloat(2.5f, 5f);
                    }
                    if (Projectile.owner == Main.myPlayer)
                    {
                        Projectile proj = Main.projectile[NewProjectileChange(Projectile.GetSource_FromThis(), Projectile.Center, vector2 * 15, ModContent.ProjectileType<CrystalBullet>(), Projectile.damage * 2, Projectile.knockBack, Projectile.owner, 0, 0, 1)];
                    }
                }
                else
                {
                    ...
                    if (Projectile.owner == Main.myPlayer)
                    {
                        Projectile proj = Main.projectile[NewProjectileChange(Projectile.GetSource_FromThis(), Projectile.Center, vector2 * 12, 94, Projectile.damage, Projectile.knockBack, Projectile.owner, 0, 0, 1)];
                    }
                }
                PlaySound(SoundID.Item9, Projectile.position);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color = new Color(132, 64, 205, 0);
            for (int A = 0; A < 2; A++)
            {
            ...
            if (Main.player[Projectile.owner].direction == 1)
            {
                ...
                Main.spriteBatch.Draw(texture2, Projectile.Center - Main.screenPosition, rectangle, color, Projectile.rotation, vector, new Vector2(Projectile.scale, Projectile.scale / 1.5f) / 1.3f, sprite, 0f);
            }
            else
            {
                ...
                Main.spriteBatch.Draw(texture2, Projectile.Center - Main.screenPosition, rectangle, color, Projectile.rotation + MathHelper.Pi, vector, new Vector2(Projectile.scale, Projectile.scale / 1.5f) / 1.3f, sprite, 0f);
            }
