...
{
    public class ShadowBook : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (Projectile.ai[0] >= player.HeldItem.useAnimation && Projectile.DProj().Bool[1])
            {
                ...
                {
                    Projectile proj = Main.projectile[NewProjectileChange(Projectile.GetSource_FromThis(), Projectile.Center, vector2.PerfectNormalize().RotatedBy(Main.rand.NextFloat(-0.4f, 0.4f)) * Main.rand.NextFloat(1.5F, 3.5F), ModContent.ProjectileType<ShadowBall>(), Projectile.damage, Projectile.knockBack, Projectile.owner, vector2.Y * 8, 0, 1)];
                }
    ...
    public class ShadowBall : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Projectile.Track(300, 20, Projectile.localAI[0], 20, false);
            NewDustChange(1, Projectile.position, new Vector2(1), 27, 1, 1);
        }
        public override void OnKill(int timeLeft)
        {
            NewDustChange(100, Projectile.position, new Vector2(1), 27, 2, 4);
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < Projectile.oldPos.Length; i++)
            {
                ...
                Color Trailcolor = color * ((Projectile.oldPos.Length - i) / (float)Projectile.oldPos.Length / 2f);
                Main.spriteBatch.Draw(texture, vector2, null, Trailcolor, Projectile.rotation, texture.Size() / 2, Projectile.scale * ((Projectile.oldPos.Length - i) / (float)Projectile.oldPos.Length), spriteEffects, 0f);
            ...
            Main.spriteBatch.Draw(texture, Projectile.position + vector - Main.screenPosition, null, new Color(132, 166, 21, 0) * 0.25F, Projectile.rotation, texture.Size() / 2, Projectile.scale/2, spriteEffects, 0f);
