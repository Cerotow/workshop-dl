    ...
    public class GreenLaser : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int A = 0; A < 30 * Projectile.scale; A++)
            {
                ...
                dust.scale = Projectile.scale;
                dust.customData = Projectile.scale;
                dust.velocity = Main.rand.NextVector2Unit() * Main.rand.NextFloat(3.5f, 7.5f) * Projectile.scale;
                dust.rotation = Projectile.rotation;
            }
        ...
        public override void OnHitNPC(NPC target, HitInfo hit, int damageDone)
        {
            ...
            for (int A = 0; A < 10* Projectile.scale; A++)
            {
                ...
                dust.scale = Projectile.scale;
                dust.customData = Projectile.scale;
                dust.velocity = Main.rand.NextVector2Unit() * Main.rand.NextFloat(1.5f, 2.5f)* Projectile.scale;
                dust.rotation = Projectile.rotation;
            }
        }
