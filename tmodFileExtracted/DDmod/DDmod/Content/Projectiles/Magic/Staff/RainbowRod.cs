...
{
    public class RainbowRod : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (player.controlUseItem)
            {
                ...
                for (int a = 0; a < 1000; a++)
                {
                    ...
                    if (projectile.type>0&& projectile.active && projectile.owner == Projectile.owner && projectile.DProj().Bool[1] && projectile.type == ModContent.ProjectileType<RainbowRodFormation>())
                    {
                        p++;
                    }
                }
                ...
                {
                    int proj = NewProjectile(Projectile.GetSource_FromThis(), player.Dplayer().MouseWorld - new Vector2(Main.rand.NextFloat(-80, 80), Main.rand.NextFloat(100, 160)), Projectile.velocity * 12, ModContent.ProjectileType<RainbowRodFormation>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI);
                ...
                if (Projectile.DProj().Times[2] >= 20)
                {
                    ...
                    {
                        Projectile projectile = Main.projectile[a];
                        if (projectile.active && projectile.owner == Projectile.owner && projectile.type == 79 && projectile.ai[1] == 0)
                        {
                            pr++;
                        }
                        if (projectile.active && projectile.owner == Projectile.owner && projectile.type == 79 && projectile.ai[0] == 0 && projectile.ai[1] == 0)
                        {
                            pr2++;
                        }
                    }
                    ...
                    {
                        TT2 = 3.5F;
                        SoundStyle sound = SoundID.Item28;
                        sound.Pitch = -1;
                        PlaySound(sound, Projectile.Center);
                        NewDustChangeRound(100, Projectile.position + new Vector2(Projectile.width / 2, 4).RotatedBy(player.fullRotation), 1, 66, 0, 12, true, 2);
                    }
                        ...
                        if (pr <= 6)
                        {
                            if (player.statMana >= player.ItemMana())
                            {
                                ...
                                if (Main.myPlayer == Projectile.owner)
                                {
                                    ...
                                    if (pr2 == 0)
                                    {
                                        int proj = NewProjectile(Projectile.GetSource_FromThis(), Projectile.position + new Vector2(Projectile.width / 2, 4).RotatedBy(player.fullRotation), Vector2.Zero, 79, Projectile.damage, 1, Projectile.owner, 0);
                                        Main.projectile[proj].netUpdate = true;
                                    }
                                    else
                                    {
                                        int proj = NewProjectile(Projectile.GetSource_FromThis(), Projectile.position + new Vector2(Projectile.width / 2, 4).RotatedBy(player.fullRotation), Vector2.Zero, 79, Projectile.damage, 1, Projectile.owner, 1);
                                        Main.projectile[proj].netUpdate = true;
                                    }
                                }
                            }
                    ...
                    if (player.ownedProjectileCounts[79] > 3 || player.statMana < player.ItemMana())
                    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.spriteBatch.Begin((SpriteSortMode)1, BlendState.Additive, Main.DefaultSamplerState, DepthStencilState.None, Main.Rasterizer, null, Main.GameViewMatrix.TransformationMatrix);
                ...
                GameShaders.Misc["渲染滤镜"].SetShaderTexture(DDTextures.彩虹);
                GameShaders.Misc["渲染滤镜"].Shader.Parameters["uImageSize1"].SetValue(DDTextures.彩虹.Size());
                GameShaders.Misc["渲染滤镜"].UseColor(Color.White);
                GameShaders.Misc["渲染滤镜"].Shader.Parameters["uColor2"].SetValue(Color.White.ToVector3()*0.5f);
                GameShaders.Misc["渲染滤镜"].Shader.Parameters["renderTargetArea"].SetValue(new Vector2(texture.Width, texture.Height));
                GameShaders.Misc["渲染滤镜"].Shader.Parameters["uWorldPosition"].SetValue(Vector2.Zero);
                GameShaders.Misc["渲染滤镜"].Shader.Parameters["position"].SetValue(new Vector2(player.Dplayer().PlayerTimes*10, player.Dplayer().PlayerTimes));
                GameShaders.Misc["渲染滤镜"].Shader.Parameters["ImageSize"].SetValue(new Vector2(DDTextures.彩虹.Width(), DDTextures.彩虹.Height()/10));
            ...
            Main.spriteBatch.Draw(Starlight, Projectile.position+ new Vector2(Projectile.width/2, 4).RotatedBy(player.fullRotation) - Main.screenPosition, null, new Color(Main.DiscoColor.R, Main.DiscoColor.G, Main.DiscoColor.B, 0), player.fullRotation, Starlight.Size() / 2, new Vector2(1, 0.5f) * 2 * Projectile.ai[1] * (TT* TT2), 0, 0f);
            Main.spriteBatch.Draw(Starlight, Projectile.position + new Vector2(Projectile.width / 2, 4).RotatedBy(player.fullRotation)-Main.screenPosition,null, new Color(255-Main.DiscoColor.R, 255-Main.DiscoColor.G, 255-Main.DiscoColor.B, 0), player.fullRotation, Starlight.Size() / 2, new Vector2(1, 0.5f) * Projectile.ai[1] * (TT* TT2), 0, 0f);
    ...
    public class RainbowRodFormation : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.DProj().Bool[1])
            {
                ...
                {
                    Projectile projectile = Main.projectile[a];
                    if (projectile.active && projectile.owner == Projectile.owner && projectile.type == ModContent.ProjectileType<RainbowRod>())
                    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color2 = Projectile.GetAlpha(new Color(Main.DiscoColor.R, Main.DiscoColor.G, Main.DiscoColor.B, 0));
            Texture2D texture = DDTextures.VoidStar.Value;
            Texture2D texture3 = DDTextures.Scanning2.Value;
            Vector2 vector = Projectile.Center - Main.screenPosition;
            Color color = Projectile.GetAlpha(new Color(Main.DiscoColor.R, Main.DiscoColor.G, Main.DiscoColor.B, 0));
