...
{
    public class 丛林权杖Proj : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (player.controlUseItem)
            {
                ...
                for (int a = 0; a < 1000; a++)
                {
                    ...
                    if (projectile.type>0&& projectile.active && projectile.owner == Projectile.owner && projectile.DProj().Bool[1] && projectile.type == ModContent.ProjectileType< 丛林权杖法阵 >())
                    {
                        p++;
                    }
                }
                ...
                {
                    int proj = NewProjectile(Projectile.GetSource_FromThis(), player.Dplayer().MouseWorld - new Vector2(Main.rand.NextFloat(-80, 80), Main.rand.NextFloat(100, 160)), Projectile.velocity * 12, ModContent.ProjectileType<丛林权杖法阵>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI);
                    ...
                    if (Projectile.ai[1] < 1)
                    {
                        ...
                        if (Projectile.ai[1] == 0.25)
                        {
                            ...
                            if (Main.netMode != 2)
                            {
                                ...
                                for (int a = 0; a < 80; a++)
                                {
                                    int dust = NewDust(player.Center + new Vector2(-texture.Width * 0.5F / 2, player.height / 2), (int)(texture.Width * 0.5F), 1, Type, Projectile.oldVelocity.X, Projectile.oldVelocity.Y, 100, new Color(0, 255, 0, 0));
                                    Main.dust[dust].noGravity = true;
                                    Main.dust[dust].scale = 1.5f;
                                    Main.dust[dust].velocity = new Vector2((Main.dust[dust].position.X - player.Center.X) / 10, -Main.rand.NextFloat(0, 6));
                            ...
                            if (Main.myPlayer == Projectile.owner)
                            {
                                if (player.statMana >= player.ItemMana() * 12)
                                {
                                    ...
                                    for (int a = 0; a < 12; a++)
                                    {
                                        ...
                                        NewProjectile(Projectile.GetSource_FromThis(), PO+ VE.PerfectNormalize()*10, VE, ModContent.ProjectileType<魔法叶>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0, 0, Main.rand.NextFloat(0.8F, 1.3F));
                                    }
                                }
                            }
                        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (player.direction == 1)
            {
                ...
                if (Projectile.ai[1] >= 0.25 && Projectile.DProj().Times[2] <= 0)
                {
                    Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, null, new Color(0, 255, 0, 0), Projectile.rotation, texture.Size() / 2, Projectile.scale, 0, 0f);
                    Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, null, new Color(0, 255, 0, 0), Projectile.rotation, texture.Size() / 2, Projectile.scale, 0, 0f);
                }
            }
            else
            {
                ...
                if (Projectile.ai[1] >= 0.25&& Projectile.DProj().Times[2] <= 0)
                {
                    Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, null, new Color(0, 255, 0, 0), Projectile.rotation + MathHelper.Pi, texture.Size() / 2, Projectile.scale, (SpriteEffects)(-1), 0f);
                    Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, null, new Color(0, 255, 0, 0), Projectile.rotation + MathHelper.Pi, texture.Size() / 2, Projectile.scale, (SpriteEffects)(-1), 0f);
                }
            }
            return false;
    ...
    public class 丛林权杖法阵 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.DProj().Bool[1])
            {
                ...
                {
                    Projectile projectile = Main.projectile[a];
                    if (projectile.active && projectile.owner == Projectile.owner && projectile.type == ModContent.ProjectileType<丛林权杖Proj>())
                    {
                ...
                Lighting.AddLight(player.Center, new Color(0, 255, 0, 150).ToVector3() * Projectile.ai[1]);
                if (Main.netMode != 2)
                {
                    Texture2D texture = DDTextures.VoidStar.Value;
                    int Type =ModContent.DustType<光球粒子>();
                    int dust = NewDust(player.Center + new Vector2(-texture.Width * 0.5F / 2 * Projectile.ai[1], player.height / 2), (int)(texture.Width * 0.5F * Projectile.ai[1]), 1, Type, Projectile.oldVelocity.X, Projectile.oldVelocity.Y, 100, new Color(0,255,0,150));
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color2 = Projectile.GetAlpha(new Color(0, 255, 0, 0));
            Texture2D texture3 = DDTextures.Scanning2.Value;
            Vector2 vector;
            Color color = Projectile.GetAlpha(new Color(0, 255, 0, 150));
            color.A = 0;
