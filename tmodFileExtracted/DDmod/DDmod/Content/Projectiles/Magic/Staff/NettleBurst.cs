...
{
    public class NettleBurst : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (player.controlUseItem && player.statMana >= player.ItemMana())
            {
                ...
                for (int a = 0; a < 1000; a++)
                {
                    ...
                    if (projectile.type>0&& projectile.active && projectile.owner == Projectile.owner && projectile.DProj().Bool[1] && !projectile.DProj().Bool[3] && projectile.type == ModContent.ProjectileType<NettleBurstFormation>())
                    {
                        p++;
                    }
                ...
                {
                    int proj = NewProjectile(Projectile.GetSource_FromThis(), player.position, new Vector2(0,-10), ModContent.ProjectileType<NettleBurstFormation>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI);
                    ...
                    else if (Projectile.ai[0] > player.ActiveItem().useAnimation)
                    {
                        ...
                        else if(!player.Dplayer().NoRight)
                        {
                            ...
                            if (Main.myPlayer == Projectile.owner)
                            {
                                for (int a = 0; a < 3; a++)
                                {
                                    int proj = NewProjectile(Projectile.GetSource_FromThis(), player.Dplayer().MouseWorld - new Vector2(0, Main.rand.NextFloat(150, 200)).RotatedBy(Main.rand.NextFloat(0, MathHelper.TwoPi)), Projectile.velocity * 20, ModContent.ProjectileType<NettleBurstFormation>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI);
                                    Main.projectile[proj].netUpdate = true;
                                }
                            } 
                        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (player.direction == 1)
            {
                if (Projectile.ai[1] < 1)
                {
                    Main.spriteBatch.Draw(Glow.Value, Projectile.Center - Main.screenPosition + new Vector2(0, player.gfxOffY), null, new Color(179,252,0, 0) * 0.5f * Projectile.ai[1], Projectile.rotation, Glow.Size() / 2, 0.25F, 0, 0f);
                    Main.spriteBatch.Draw(Glow.Value, Projectile.Center - Main.screenPosition + new Vector2(0, player.gfxOffY), null, new Color(179,252,0, 0) * 0.5f * Projectile.ai[1], Projectile.rotation, Glow.Size() / 2, 0.25F, 0, 0f);
                }
                else
                {
                    Main.spriteBatch.Draw(Glow.Value, Projectile.Center - Main.screenPosition + new Vector2(0, player.gfxOffY), null, new Color(179,252,0, 0) * Projectile.ai[1], Projectile.rotation, Glow.Size() / 2, 0.25F, 0, 0f);
                    Main.spriteBatch.Draw(Glow.Value, Projectile.Center - Main.screenPosition + new Vector2(0, player.gfxOffY), null, new Color(179,252,0, 0) * Projectile.ai[1], Projectile.rotation, Glow.Size() / 2,0.25F, 0, 0f);
                }
            ...
            else
            {
                if (Projectile.ai[1] < 1)
                {
                    Main.spriteBatch.Draw(Glow.Value, Projectile.Center - Main.screenPosition +new Vector2(0,player.gfxOffY), null, new Color(179,252,0, 0) * 0.5f * Projectile.ai[1], Projectile.rotation + MathHelper.Pi, Glow.Size() / 2,0.25F, (SpriteEffects)(-1), 0f);
                    Main.spriteBatch.Draw(Glow.Value, Projectile.Center - Main.screenPosition + new Vector2(0, player.gfxOffY), null, new Color(179,252,0, 0) * 0.5f * Projectile.ai[1], Projectile.rotation + MathHelper.Pi, Glow.Size() / 2,0.25F, (SpriteEffects)(-1), 0f);
                }
                else
                {
                    Main.spriteBatch.Draw(Glow.Value, Projectile.Center - Main.screenPosition + new Vector2(0, player.gfxOffY), null, new Color(179,252,0, 0) * Projectile.ai[1], Projectile.rotation + MathHelper.Pi, Glow.Size() / 2,0.25F, (SpriteEffects)(-1), 0f);
                    Main.spriteBatch.Draw(Glow.Value, Projectile.Center - Main.screenPosition + new Vector2(0, player.gfxOffY), null, new Color(179,252,0, 0) * Projectile.ai[1], Projectile.rotation + MathHelper.Pi, Glow.Size() / 2,0.25F, (SpriteEffects)(-1), 0f);
                }
                Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition + new Vector2(0, player.gfxOffY), null, Color.White, Projectile.rotation + MathHelper.Pi, texture.Size() / 2, Projectile.scale, (SpriteEffects)(-1), 0f);
            }
            Texture2D Starlight = DDTextures.Starlight.Value;
            DDHelper.BackAndForth(0.4F, 0.6F, 0.04F, ref Sc, ref ScB);
            Main.spriteBatch.Draw(Starlight, Projectile.position + new Vector2(Projectile.width/2, -6).RotatedBy(player.fullRotation) - Main.screenPosition, null, new Color(200, 50, 0, 0), player.fullRotation, Starlight.Size() / 2, new Vector2(1, 0.5f) * 2 * Projectile.ai[1] * Sc* Projectile.DProj().Times[4], 0, 0f);
            Main.spriteBatch.Draw(Starlight, Projectile.position + new Vector2(Projectile.width/2, -6).RotatedBy(player.fullRotation) - Main.screenPosition, null, new Color(55, 105, 155, 0), player.fullRotation, Starlight.Size() / 2, new Vector2(1, 0.5f) * Projectile.ai[1] * Sc* Projectile.DProj().Times[4], 0, 0f);
    ...
    public class NettleBurstFormation : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.DProj().Bool[1])
            {
                ...
                {
                    Projectile projectile = Main.projectile[a];
                    if (projectile.active && projectile.owner == Projectile.owner && projectile.type == ModContent.ProjectileType<NettleBurst>())
                    {
                ...
                Lighting.AddLight(player.Center, new Color(179,252,0).ToVector3() * Projectile.ai[1]);
                if (Main.netMode != 2)
                {
                    Texture2D texture = DDTextures.VoidStar.Value;
                    int Type = 163;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color2 = Projectile.GetAlpha(new Color(209, 88, 60, 0));
            Texture2D texture = DDTextures.VoidStar.Value;
            Texture2D texture3 = DDTextures.Scanning2.Value;
            Vector2 vector = Projectile.Center - Main.screenPosition;
            Color color = Projectile.GetAlpha(new Color(209, 88, 60, 0));
            ...
            if (Projectile.DProj().Times[2] < 80)
            {
                ...
                Main.spriteBatch.Draw(texture3, vector + Projectile.rotation.ToRotationVector2() * (10 * Projectile.scale), null, new Color(155, 55, 155, 0), Projectile.rotation + MathHelper.PiOver2, texture3.Size() / 2, new Vector2(Projectile.scale * 1.2F, Projectile.scale) / 2, 0, 0f);
            }
            ...
            if (Projectile.DProj().Bool[1])
            {
                color2 = Projectile.GetAlpha(new Color(179, 252, 0, 0));
