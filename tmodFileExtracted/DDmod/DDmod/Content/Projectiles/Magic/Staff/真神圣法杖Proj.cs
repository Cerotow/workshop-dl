...
{
    public class 真神圣法杖Proj : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (player.controlUseItem && player.statMana >= player.ItemMana())
            {
                ...
                for (int a = 0; a < 1000; a++)
                {
                    ...
                    if (projectile.type>0&& projectile.active && projectile.owner == Projectile.owner && projectile.DProj().Bool[1] && projectile.type == ModContent.ProjectileType<TrueHallowedFormation>())
                    {
                        p++;
                    }
                }
                if (p == 0)
                {
                    ...
                    {
                        int proj = NewProjectile(Projectile.GetSource_FromThis(), player.Dplayer().MouseWorld - new Vector2(Main.rand.NextFloat(-80, 80), Main.rand.NextFloat(100, 160)), Projectile.velocity * 12, ModContent.ProjectileType<TrueHallowedFormation>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI);
                ...
                else if (Projectile.ai[0] > player.ActiveItem().useAnimation && Main.myPlayer == Projectile.owner)
                {
                    ...
                    for (int a = 0; a < 1; a++)
                    {
                        int proj = NewProjectile(Projectile.GetSource_FromThis(), player.Dplayer().MouseWorld - new Vector2(Main.rand.NextFloat(-280, 280), Main.rand.NextFloat(300, 460)), Projectile.velocity * 20, ModContent.ProjectileType<TrueHallowedFormation>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI);
                        Main.projectile[proj].netUpdate = true;
                    }
                }
            }
    ...
    public class TrueHallowedFormation : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.DProj().Bool[1])
            {
                ...
                {
                    Projectile projectile = Main.projectile[a];
                    if (projectile.active && projectile.owner == Projectile.owner && projectile.type == ModContent.ProjectileType<真神圣法杖Proj>())
                    {
                ...
                if (Main.projectile[(int)Projectile.ai[0]].active && Main.projectile[(int)Projectile.ai[0]].type == ModContent.ProjectileType<真神圣法杖Proj>())
                {
                ...
                if (Main.netMode != 2)
                {
                    ...
                    dust.color = new Color(236, 220, 109, 0);
                    if (Main.rand.NextBool(2))
                    {
                        dust.color = new Color(221, 30, 96, 0);
                    }
                ...
                if (!Projectile.DProj().Bool[0])
                {
                    ...
                    if (Projectile.scale > 1F)
                    {
                        if (Main.myPlayer == Projectile.owner)
                        {
                            ...
                            {
                                NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.rotation.ToRotationVector2().RotatedBy(Main.rand.NextFloat(-0.5F, 0F)) * 12, ModContent.ProjectileType<魔法神圣弹>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI, 1);
                                NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.rotation.ToRotationVector2().RotatedBy(Main.rand.NextFloat(0F, 0.5F)) * 12, ModContent.ProjectileType<魔法神圣弹>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI, 4);
                            ...
                            else
                            {
                                ...
                                int Proj = NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.rotation.ToRotationVector2() * 12, ModContent.ProjectileType<魔法神圣弹>(), Projectile.damage*2, 0, Projectile.owner, Projectile.whoAmI, 3);
                                Main.projectile[Proj].DProj().Magnification = 2;
                                NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.rotation.ToRotationVector2().RotatedBy(Main.rand.NextFloat(-0.5F, 0.5F)) * 12, ModContent.ProjectileType<魔法神圣弹>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI, 4);
                            }
                        }
                        int Type = ModContent.DustType<光球粒子>();
                        for (int A = 0; A < 80; A++)
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color2 = Projectile.GetAlpha(new Color(221, 3, 96, 0));
            ...
            Main.spriteBatch.Draw(texture3, vector + Projectile.rotation.ToRotationVector2() * (10 * Projectile.scale), null, color, Projectile.rotation + MathHelper.PiOver2, texture3.Size() / 2, new Vector2(Projectile.scale * 1.2F, Projectile.scale)/2, 0, 0f);
            ...
            if (Projectile.DProj().Bool[1])
            {
                ...
                Main.spriteBatch.Draw(texture, vector, null, color2, player.fullRotation + MathHelper.PiOver2, texture.Size() / 2, new Vector2(0.75F * Projectile.ai[1] / 3, Projectile.ai[1]), 0, 0f);
                Main.spriteBatch.Draw(texture, vector, null, color * 0.5F, player.fullRotation + MathHelper.PiOver2, texture.Size() / 2, new Vector2(0.75F * Projectile.ai[1] / 3,  Projectile.ai[1]), 0, 0f);
                Main.spriteBatch.Draw(texture3, vector - new Vector2(0, 10).RotatedBy(player.fullRotation), null, color * 0.7f, player.fullRotation, texture3.Size() / 2, new Vector2(1.2F, 1) * Projectile.ai[1], 0, 0f);
                Main.spriteBatch.Draw(texture3, vector - new Vector2(0, 10).RotatedBy(player.fullRotation), null, color2 * 0.7f, player.fullRotation, texture3.Size() / 2, new Vector2(0.8F, 1) * Projectile.ai[1], 0, 0f);
                ...
                Main.spriteBatch.Draw(Starlight, vector - new Vector2(0, 40).RotatedBy(player.fullRotation), null, Projectile.GetAlpha(new Color(221, 3, 96, 50)), player.fullRotation, Starlight.Size() / 2, new Vector2(1, 0.5f) * 1.5f * Projectile.ai[1] * Projectile.localAI[0], 0, 0f);
                ...
                Main.EntitySpriteDraw(texture2, vector, new Rectangle?(new Rectangle(0, 0, texture2.Width, texture2.Height)), color, 0f, Utils.Size(texture2) * 0.5f, Projectile.ai[1], 0, 0);
                Main.spriteBatch.End();
                Main.spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, Main.DefaultSamplerState, DepthStencilState.None, Main.Rasterizer, null, Main.GameViewMatrix.TransformationMatrix);
            }
            return false;
