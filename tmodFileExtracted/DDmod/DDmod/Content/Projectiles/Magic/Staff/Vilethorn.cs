...
{
    public class Vilethorn : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            {
                Projectile projectile = Main.projectile[NewProjectileChange(Projectile.GetSource_FromThis(), Projectile.Center + Projectile.velocity.PerfectNormalize() * Projectile.SolidTileDistanceDetection(100, 50), vector * (player.ActiveItem().shootSpeed + Projectile.ai[1]), ModContent.ProjectileType<VilethornBullet>(), Projectile.damage, Projectile.knockBack, Projectile.owner, player.GetWeaponDamage(player.HeldItem), 0, Projectile.ai[1])];
                projectile.scale = Projectile.ai[1] * 0.75f;
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Player player = Main.player[Projectile.owner];
            Color color = new Color(68, 97, 37)*0.75f;
            color.A = 255;
            ...
            Main.spriteBatch.Draw(texture3, vector2, null, color * 0.35f, Projectile.rotation - MathHelper.PiOver4, texture3.Size() / 2, new Vector2(Projectile.ai[1] / 6, Projectile.ai[1] / 2.5f), 0, 0f);
            ...
            DDHelper.DrawExpanded(Main.GameViewMatrix.TransformationMatrix, Main.spriteBatch, player.Center - Main.screenPosition + new Vector2(0, -50), 0.5f, Projectile.ai[1] / player.ActiveItem().MagicItem().charging * 0.5f, color * 2f, color, 1.1F, 0.6f);
