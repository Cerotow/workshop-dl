...
{
    public class ShadowFlameStaff : AMagicStaff
    {
        ...
        public override void ShootEffect(Player player)
        {
            ...
            for (int a = 0; a < 40; a++)
            {
                int Dust = NewDust(vector + new Vector2(0, Main.rand.NextFloat(-40, 40)).RotatedBy(Projectile.rotation - StaffRot), 0, 0, 27, 0, 0, 0, Color.White, Main.rand.NextFloat(0.4F, 1.2F));
    ...
    public class ShadowFireball : ModProjectile
    {
        ...
        {
            //DisplayName.SetDefault("Shadow Fireball");
            //DisplayName.AddTranslation(7, "暗影火球");
        ...
        public override void OnKill(int timeLeft)
        {
            int Proj=NewProjectile(Projectile.GetSource_FromThis(), Projectile.position + new Vector2(Projectile.width / 2, Projectile.height), Vector2.Zero, ModContent.ProjectileType<ShadowFlamePillar>(), Projectile.damage / 2, Projectile.knockBack / 2, Projectile.owner);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, null, new Color(81, 6, 233, 0), Projectile.rotation, texture.Size() / 2, Projectile.scale/2, spriteEffects, 0f);
            Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, null, new Color(81, 6, 233, 0), Projectile.rotation, texture.Size() / 2, Projectile.scale/2, spriteEffects, 0f);

            return false;
        }
    }
    public class ShadowFlamePillar : ModProjectile
    {
        public override void SetStaticDefaults()
        {
           //DisplayName.SetDefault("Shadow Flame Pillar");
           //DisplayName.AddTranslation(7, "暗影焰柱");
        }
        ...
        public override void AI()
        {
            for (int A = 0; A < 2 + Projectile.height / 80; A++)
            {
                ...
                dust.noGravity = true;
                dust.scale = 2;
                dust.velocity = new Vector2(0, -1).RotatedBy(Main.rand.NextFloat(-0.6f, 0.6f)) * Main.rand.NextFloat(5, 25);
            }
            if (Main.rand.NextBool(30))
            {
                int A = NewProjectile(Projectile.GetSource_FromThis(), Projectile.position + new Vector2(Projectile.width, Projectile.height), new Vector2(0, -1).RotatedBy(Main.rand.NextFloat(-0.6f, 0.6f)) * Main.rand.NextFloat(5, 8), ModContent.ProjectileType<ShadowBullet>(), Projectile.damage / 2, Projectile.knockBack / 2, Projectile.owner, 1);
                Main.projectile[A].scale = 0.2f;
                Main.projectile[A].friendly = Projectile.friendly;
                Main.projectile[A].hostile = Projectile.hostile;
            }
            if (Projectile.hostile)
            {
                ...
                {
                    NPC npc = Main.npc[NewNPC(Projectile.GetSource_FromThis(), (int)Projectile.Center.X+16, (int)Projectile.Center.Y, 29, 0, 1, 20)];
