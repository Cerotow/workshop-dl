...
{
    public class BatScepter : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (player.controlUseItem && player.statMana >= player.ItemMana())
            {
                ...
                for (int a = 0; a < 1000; a++)
                {
                    ...
                    if (projectile.type > 0 && projectile.active && projectile.owner == Projectile.owner && !projectile.DProj().Bool[3] && projectile.type == ModContent.ProjectileType<BatScepterFormation>())
                    {
                        p++;
                    }
                ...
                if (p == 0 && Projectile.ai[1] > 0.25F)
                {
                    ...
                    {
                        int proj = NewProjectile(Projectile.GetSource_FromThis(), player.position, new Vector2(0, -10), ModContent.ProjectileType<BatScepterFormation>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI);
                ...
                if (Projectile.DProj().Times[2] <= 0)
                {
                    ...
                    else if (Projectile.ai[0] > player.ActiveItem().useAnimation)
                    {
                        ...
                        else if( !player.Dplayer().NoRight)
                        {
                            ...
                            if (Main.myPlayer == Projectile.owner)
                            {
                                int proj = NewProjectile(Projectile.GetSource_FromThis(), player.Dplayer().MouseWorld - new Vector2(0, Main.rand.NextFloat(200)).RotatedBy(Main.rand.NextFloat(0, MathHelper.TwoPi)), Vector2.Zero, 316, Projectile.damage, 0, Projectile.owner, Projectile.whoAmI);
                                Main.projectile[proj].netUpdate = true;
                            }
                        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(Starlight, Projectile.position + new Vector2(Projectile.width / 2, 8).RotatedBy(player.fullRotation) - Main.screenPosition, null, new Color(46, 43, 45, 255), player.fullRotation, Starlight.Size() / 2, new Vector2(1, 0.5f) * 2 * Projectile.ai[1] * Sc * Projectile.DProj().Times[4], 0, 0f);
            Main.spriteBatch.Draw(Starlight, Projectile.position + new Vector2(Projectile.width / 2, 8).RotatedBy(player.fullRotation) - Main.screenPosition, null, new Color(46, 43, 45,255).Opposite(), player.fullRotation, Starlight.Size() / 2, new Vector2(1, 0.5f) * Projectile.ai[1] * Sc * Projectile.DProj().Times[4], 0, 0f);
    ...
    public class BatScepterFormation : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                Projectile projectile = Main.projectile[a];
                if (projectile.active && projectile.owner == Projectile.owner && projectile.type == ModContent.ProjectileType<BatScepter>())
                {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color = Projectile.GetAlpha(new Color(0, 0, 0,255));
            Color color2 = Projectile.GetAlpha(new Color(46, 43, 45, 0));
            Texture2D texture = DDTextures.VoidStar.Value;
            ...
            for (int A = 0; A < R.Length; A+=2)
            {
                    Main.spriteBatch.Draw(BatScepter.Glow.Value, player.MountedCenter - new Vector2(6, 0) - Main.screenPosition, null, new Color(0, 0, 0, 255)* (1f-(float)A/R.Length), R[A], new Vector2(BatScepter.Glow.Width(), 130), Projectile.ai[2] * new Vector2(1.5f, 1), 0, 0f);
                    Main.spriteBatch.Draw(BatScepter.Glow.Value, player.MountedCenter + new Vector2(6, 0) - Main.screenPosition, null, new Color(0, 0, 0, 255) * (1f - (float)A / R.Length), -R[A], new Vector2(0, 130), Projectile.ai[2]*new Vector2(1.5f,1), SpriteEffects.FlipHorizontally, 0);
                    Main.spriteBatch.Draw(BatScepter.Glow.Value, player.MountedCenter - new Vector2(6, 0) - Main.screenPosition, null, new Color(0, 0, 0, 255)* (1f-(float)A/R.Length), R[A], new Vector2(BatScepter.Glow.Width(), 130), Projectile.ai[2] * new Vector2(1.5f, 1)*1.25f, 0, 0f);
                    Main.spriteBatch.Draw(BatScepter.Glow.Value, player.MountedCenter + new Vector2(6, 0) - Main.screenPosition, null, new Color(0, 0, 0, 255) * (1f - (float)A / R.Length), -R[A], new Vector2(0, 130), Projectile.ai[2]*new Vector2(1.5f,1) * 1.25f, SpriteEffects.FlipHorizontally, 0);
            ...
            DDHelper.Compression(texture2, color, player.fullRotation + MathHelper.PiOver2, Projectile.Opacity, new Vector2(4, 1), Projectile.direction, Projectile.DProj().Times[1], BlendState.AlphaBlend);
