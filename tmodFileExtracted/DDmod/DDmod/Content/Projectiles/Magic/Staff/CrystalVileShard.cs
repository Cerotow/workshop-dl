...
{
    public class CrystalVileShard : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            for (int A = 0; A < 15; A++)
            {
                ...
                Vector2 vector = Projectile.velocity.RotatedBy(Main.rand.NextFloat(-0.6F, 0.6F));
                Dust dust = Main.dust[NewDust(Projectile.Center + new Vector2(Main.rand.NextFloat(-12, 12) * Projectile.ai[1], 0).RotatedBy(Projectile.rotation + MathHelper.PiOver4) - new Vector2(4) + Projectile.velocity.PerfectNormalize() * 24, 1, 1, Type, 0, 0, 100, default)];
                dust.noGravity = true;
                dust.scale = 0.3F* Projectile.ai[1];
                dust.velocity = vector * Main.rand.NextFloat(2, 18);
            }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            {
                Projectile projectile = Main.projectile[NewProjectileChange(Projectile.GetSource_FromThis(), Projectile.Center + Projectile.velocity.PerfectNormalize() * Projectile.SolidTileDistanceDetection(100, 50), vector * (player.ActiveItem().shootSpeed + Projectile.ai[1]), ModContent.ProjectileType<CrystalThorn>(), Projectile.damage, Projectile.knockBack, Projectile.owner, player.GetWeaponDamage(player.HeldItem), 0, Projectile.ai[1])];
                projectile.scale = Projectile.ai[1] * 0.75f;
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Player player = Main.player[Projectile.owner];
            Color color = new Color(191, 56, 164, 0);
            ...
            texture = DDTextures.Circle[11].Value;
            DDHelper.Compression(texture, color, Projectile.rotation - MathHelper.PiOver4, Projectile.Opacity, new Vector2(2 + Projectile.ai[1], 1), Projectile.direction, Projectile.DProj().Times[0], BlendState.AlphaBlend);
            ...
            Main.spriteBatch.Draw(texture, vector2 - Projectile.velocity.PerfectNormalize() * 30 * T3, new Rectangle?(new Rectangle(0, 0, texture.Width, texture.Height)), color, 0f, Utils.Size(texture) / 2, Projectile.ai[1] / 3 * (1 - T3), 0, 0);
            texture = DDTextures.Circle[11].Value;
            color = new Color(191, 56, 164, 0)*0.5f;
            ...
            texture = DDTextures.Circle[8].Value;
            color = new Color(56, 126, 191, 0) * 0.5f;
            DDHelper.Compression(texture, color, Projectile.rotation - MathHelper.PiOver4, Projectile.Opacity, new Vector2(2 + Projectile.ai[1], 1), Projectile.direction, Projectile.DProj().Times[0], BlendState.Additive);
            Main.spriteBatch.Draw(texture, vector2, new Rectangle?(new Rectangle(0, 0, texture.Width, texture.Height)), color, 0f, Utils.Size(texture) / 2, Projectile.ai[1] / 3, 0, 0);
            Main.spriteBatch.Draw(texture, vector2 - Projectile.velocity.PerfectNormalize() * 30 * T1, new Rectangle?(new Rectangle(0, 0, texture.Width, texture.Height)), color, 0f, Utils.Size(texture) / 2, Projectile.ai[1] / 3 * (1 - T1), 0, 0);
            Main.spriteBatch.Draw(texture, vector2 - Projectile.velocity.PerfectNormalize() * 30 * T2, new Rectangle?(new Rectangle(0, 0, texture.Width, texture.Height)), color, 0f, Utils.Size(texture) / 2, Projectile.ai[1] / 3 * (1 - T2), 0, 0);
            ...
            DDHelper.DrawExpanded(Main.GameViewMatrix.TransformationMatrix, Main.spriteBatch, player.Center - Main.screenPosition + new Vector2(0, -50), 0.5f, Projectile.ai[1] / player.ActiveItem().MagicItem().charging * 0.5f, color , color, 1.1F, 0.6f);
