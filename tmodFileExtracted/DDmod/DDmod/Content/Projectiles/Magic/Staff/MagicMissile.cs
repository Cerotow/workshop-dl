...
namespace DDmod.Content.Projectiles.Magic.Staff
{
    public class MagicMissile : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            if (player.controlUseItem)
            {
                ...
                for (int a = 0; a < 1000; a++)
                {
                    ...
                    if (projectile.type>0&& projectile.active && projectile.owner == Projectile.owner && projectile.DProj().Bool[1] && projectile.type == ModContent.ProjectileType<MagicMissileFormation>())
                    {
                        p++;
                    }
                }
                ...
                {
                    int proj = NewProjectile(Projectile.GetSource_FromThis(), player.Dplayer().MouseWorld - new Vector2(Main.rand.NextFloat(-80, 80), Main.rand.NextFloat(100, 160)), Projectile.velocity * 12, ModContent.ProjectileType<MagicMissileFormation>(), Projectile.damage, 0, Projectile.owner, Projectile.whoAmI);
                ...
                if (Projectile.DProj().Times[2] >= 20)
                {
                    ...
                    {
                        Projectile projectile = Main.projectile[a];
                        if (projectile.active && projectile.owner == Projectile.owner && projectile.type == 16 && projectile.ai[1] == 0)
                        {
                            pr++;
                        }
                        if (projectile.active && projectile.owner == Projectile.owner && projectile.type == 16 && projectile.ai[0] == 0&& projectile.ai[1] == 0)
                        {
                            pr2++;
                        }
                    }
                    ...
                    {
                        TT2 = 3.5F;
                        NewDustChangeRound(100, Projectile.position + new Vector2(Projectile.width / 2, 4).RotatedBy(player.fullRotation), 1, 15, 0, 5,true,2);
                    }
                        ...
                        if (pr <= 3)
                        {
                            if (player.statMana >= player.ItemMana())
                            {
                                ...
                                if (Main.myPlayer == Projectile.owner)
                                {
                                    if (pr2 == 0)
                                    {
                                        int proj = NewProjectile(Projectile.GetSource_FromThis(), Projectile.position + new Vector2(Projectile.width / 2, 4).RotatedBy(player.fullRotation), Vector2.Zero, 16, Projectile.damage, 1, Projectile.owner, 0);
                                        Main.projectile[proj].netUpdate = true;
                                    }
                                    else
                                    {
                                        int proj = NewProjectile(Projectile.GetSource_FromThis(), Projectile.position + new Vector2(Projectile.width / 2, 4).RotatedBy(player.fullRotation), Vector2.Zero, 16, Projectile.damage, 1, Projectile.owner, 1);
                                        Main.projectile[proj].netUpdate = true;
                                    }
                                }
                            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(Starlight, Projectile.position+ new Vector2(Projectile.width/2, 4).RotatedBy(player.fullRotation) - Main.screenPosition, null, new Color(69, 189, 249, 0), player.fullRotation, Starlight.Size() / 2, new Vector2(1, 0.5f) * 2 * Projectile.ai[1] * (TT* TT2), 0, 0f);
            Main.spriteBatch.Draw(Starlight, Projectile.position + new Vector2(Projectile.width / 2, 4).RotatedBy(player.fullRotation)-Main.screenPosition,null, new Color(69, 189, 249, 0), player.fullRotation, Starlight.Size() / 2, new Vector2(1, 0.5f) * Projectile.ai[1] * (TT* TT2), 0, 0f);
    ...
    public class MagicMissileFormation : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.DProj().Bool[1])
            {
                ...
                {
                    Projectile projectile = Main.projectile[a];
                    if (projectile.active && projectile.owner == Projectile.owner && projectile.type == ModContent.ProjectileType<MagicMissile>())
                    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color2 = Projectile.GetAlpha(new Color(69, 189, 249, 0));
            Texture2D texture = DDTextures.VoidStar.Value;
            Texture2D texture3 = DDTextures.Scanning2.Value;
            Vector2 vector = Projectile.Center - Main.screenPosition;
            Color color = Projectile.GetAlpha(new Color(69, 189, 249, 0));
