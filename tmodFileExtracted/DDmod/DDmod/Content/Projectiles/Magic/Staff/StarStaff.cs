...
{
    public class StarStaff : AMagicStaff
    {
        public override int ProjShoot => ModContent.ProjectileType<MagicStar2>();
        ...
        public override void ShootEffect(Player player)
        {
            ...
            for (int a = 0; a < 30; a++)
            {
                ...
                R.X /= 5;
               int Dust =  NewDust(vector + R.RotatedBy(Projectile.rotation - MathHelper.PiOver4)*10, 0, 0, ModContent.DustType<光球粒子>(), 0, 0, 100, new Color(0,200,255, 0), 0.6F);
            ...
            for (int a = 0; a < 30; a++)
            {
                int Dust = NewDust(vector + new Vector2(0, Main.rand.NextFloat(-50, 50)).RotatedBy(Projectile.rotation - MathHelper.PiOver4), 0, 0, ModContent.DustType<光球粒子>(), 0, 0, 100, new Color(0, 100, 255, 255), Main.rand.NextFloat(0.4F, 1f));
                Main.dust[Dust].velocity = Projectile.velocity.RotatedBy(Main.rand.NextFloat(-0.8F, 0.8F)) * Main.rand.NextFloat(0, ProjShootSpeed);
                Main.dust[Dust].customData = 2F;
            }
            for (int a = 0; a < 5; a++)
            {
                int Dust = NewDust(vector + new Vector2(0, Main.rand.NextFloat(-50, 50)).RotatedBy(Projectile.rotation - MathHelper.PiOver4), 0, 0, ModContent.DustType<星星粒子>(), 0, 0, 100, new Color(0, 100, 255, 255), Main.rand.NextFloat(0.4F, 1f));
                Main.dust[Dust].velocity = Projectile.velocity.RotatedBy(Main.rand.NextFloat(-0.8F, 0.8F)) * Main.rand.NextFloat(0, ProjShootSpeed)/2;
            }
            Circle = MaxCircle;
            Circle *= 1.75F;
            CircleA = 1;
        ...
        public override void Shoot(Player player)
        {
            ...
            for (int a = -1; a <= 1; a++)
            {
                if (a != 0 && Main.myPlayer == Projectile.owner)
                {
                    Projectile projectile = Main.projectile[NewProjectileChange(Projectile.GetSource_FromThis(), Projectile.Center + Projectile.velocity.PerfectNormalize() * Projectile.SolidTileDistanceDetection(80, 50), vector2 * 8, ProjShoot, Projectile.damage, Projectile.knockBack, Projectile.owner, Math.Abs(a / Rand), 0, 1)];
                    projectile.scale = 1;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color color = Projectile.GetAlpha(new Color(0,200,255, 255));
            ...
            //法阵
            DDHelper.Compression(texture, color, Projectile.rotation - MathHelper.PiOver4, Projectile.Opacity, new Vector2(5, 1), Projectile.direction, Projectile.DProj().Times[0], BlendState.AlphaBlend);
