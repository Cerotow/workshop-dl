namespace DDmod.Content.Projectiles.Summon
{
    public class ShadowFireBall : ModProjectile
    {
        ...
        {
            int A = NewProjectileChange(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity.PerfectNormalize(), ModContent.ProjectileType<ShadowFireballExplosion>(), (int)(Projectile.damage * 0.75f), Projectile.knockBack, Projectile.owner, 0, 0);
        ...
        public override bool PreKill(int timeLeft)
        {
            ...
            for (float A = 0; A < Projectile.scale; A += 0.01f)
            {
                ...
                dust.noGravity = true;
                dust.scale *= 2;
                dust.velocity = Main.rand.NextVector2Unit() * Main.rand.NextFloat(1.5f * Projectile.scale, 3.5f * Projectile.scale);
            }
            return true;
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if (Projectile.localAI[0] == 0)
            {
                ...
                Main.spriteBatch.Draw(texture, Projectile.position + vector - Main.screenPosition, null, new Color(81, 6, 233, 0), Projectile.rotation, texture.Size() / 2, Projectile.scale / 5, spriteEffects, 0f);
                ...
                for (int i = 0; i < 3; i++)
                {
                    Main.spriteBatch.Draw(texture, Projectile.position + vector - Main.screenPosition + new Vector2(2 * Projectile.scale).RotatedBy(MathHelper.TwoPi / 3 * i + Projectile.timeLeft / 10), null, new Color(81, 6, 233, 0), Projectile.rotation, texture.Size() / 2, Projectile.scale / 5, spriteEffects, 0f);
                }
            }
            return false;
        }
