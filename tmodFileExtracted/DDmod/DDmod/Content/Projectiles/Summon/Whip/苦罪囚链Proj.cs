...
{
    public class 苦罪囚链Proj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (num5 > 0.1f && Main.rand.NextFloat() < num5 / 2f)
            {
                ...
                int num6 = NewDust(r4.TopLeft(), r4.Width, r4.Height, 27, 0f, 0f, 100, default, 1f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color originalColor = new Color(81, 6, 233);
        ...
        public Vector2 DrawWhip_HeartWhip(Projectile projectile, List<Vector2> list)
        {
            ...
            for (int i = 0; i < list.Count - 1; i++)
            {
                ...
                if (flag)
                {
                    ...
                    Main.spriteBatch.Draw(value, vector2 - Main.screenPosition, (Rectangle?)rectangle, color, rotation, origin, scale, projectile.velocity.X>0?0:SpriteEffects.FlipHorizontally, 0f);
                }
                vector2 += vector4;
            }
            return vector2;
        ...
        {
            NewDustChange(30, target.Center - new Vector2(4), new Vector2(1), 27, 0, 4, false, 1.2F);
            if (rR)
            {
                Main.player[Projectile.owner].MinionAttackTargetNPC = target.whoAmI;
                if (Main.player[Projectile.owner].ownedProjectileCounts[ModContent.ProjectileType<诅咒之魂>()] == 0)
                {
                    Projectile.NewProjectileChange(target.position + new Vector2(target.width / 2, -120), Vector2.Zero, ModContent.ProjectileType<诅咒之魂>(), Projectile.damage * 2, 0, -1, 0, 0, target.whoAmI);
                }
                rR = false;
            }
            Projectile.damage /= 2;
