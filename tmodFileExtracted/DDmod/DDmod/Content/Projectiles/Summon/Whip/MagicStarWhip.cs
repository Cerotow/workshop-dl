...
{
    public class MagicStarWhip : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (num5 > 0.1f && Main.rand.NextFloat() < num5 / 2f)
            {
                ...
                int num6 = NewDust(r4.TopLeft(), r4.Width, r4.Height, ModContent.DustType<星星粒子>(), 0f, 0f, 100, default, 1f);
        ...
        {
            Main.player[Projectile.owner].AddBuff(ModContent.BuffType<MagicStar>(), 300);
            Projectile.damage /= 2;
            Main.player[Projectile.owner].MinionAttackTargetNPC = target.whoAmI;
            target.buffImmune[ModContent.BuffType<MagicStar>()] = false;
            target.AddBuff(ModContent.BuffType<MagicStar>(), 300);
        }
        ...
        public void NDust(Texture2D texture, Vector2 Position)
        {
            ...
            for (int i = 0; i < colors.Length; i++)
            {
                ...
                if (DustPos >= 0)
                {
                    Dust dust =Main.dust[NewDust(Position+ (new Vector2(x, y) - texture.Size() / 2), 1, 1, ModContent.DustType<星星粒子>(), 0, 0, 0, default, 0.5f)];
                    //dust.customData = true;
                    dust.velocity = ((new Vector2(x, y) - texture.Size() / 2)) *0.5f;
                }
            }
        }
