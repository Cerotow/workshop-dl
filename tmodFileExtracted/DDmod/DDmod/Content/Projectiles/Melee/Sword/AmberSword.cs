...
{
    public class AmberSword : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            Projectile.HoldProj(player, 34 * Projectile.scale, Projectile.ai[0], Projectile.DProj().vector[0], MathHelper.PiOver4, 0, true, 0.05f * Projectile.DProj().Times[0], false, false);
            ...
            if (Projectile.localAI[1] >= 0 && Projectile.ai[1] == 2 && Projectile.DProj().Times[2] < useTime)
            {
                ...
                for(int A = 0;A< Main.rand.Next(6); A++)
                NewProjectile(Projectile.GetSource_FromAI(), player.Center, Projectile.DProj().vector[0].RotatedBy(Main.rand.NextFloat(-0.5F,0.5F))* Main.rand.NextFloat(8F, 16F), ModContent.ProjectileType<琥珀镰刃>(), Projectile.damage / 3, Projectile.knockBack);
            }
        ...
        Color color = new Color(255, 155, 48, 120) * 0.67f;
        public override void OnHitNPC(NPC target, HitInfo hit, int damageDone)
        {
            ...
            target.AddBuff(ModContent.BuffType<Fossil>(), 120);
            if (target.realLife > 0)
            {
                Main.npc[target.realLife].AddBuff(ModContent.BuffType<Fossil>(), 120);
            }
        }
        public float TWidth()
        {
