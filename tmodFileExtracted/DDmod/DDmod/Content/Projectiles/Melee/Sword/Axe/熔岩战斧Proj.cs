...
{
    public class 熔岩战斧Proj : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            Projectile.MeleeProj().oldVels2 = TextureAssets.Item[item.type].Size().Length() / 2 * 1.5f - 6;
            ...
            if (sp2 % 12 == 0)
            {
                int A = NewDust(Projectile.Center + Projectile.velocity.PerfectNormalize() * 10 * Projectile.scale - new Vector2(20, 20), 40, 40, 6, Projectile.velocity.X * 4, Projectile.velocity.Y * 4, 0, default, 2F);
            ...
            if (sp2 % 4 == 0)
            {
                if (Main.netMode != 2)
                {
                    ...
                    for (int a = 0; a <= Projectile.width / 16; a++)
                    {
                        for (int b = 0; b <= Projectile.height / 16; b++)
                        {
            ...
            //手持弹幕
            Projectile.HoldProj(player, 46 * Projectile.scale, Projectile.ai[0] , Projectile.DProj().vector[0].RotatedBy(-3*player.direction), MathHelper.PiOver4, 0, true, sp/10* Projectile.DProj().Times[0], false, false);
            //玩家按着左键
        ...
        {
            return 16 * Projectile.scale;
        }
        internal Trailing TrailDrawer;
        Color color = new Color(244, 102, 15, 0) * 0.5f;
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(texture, Projectile.position - Main.screenPosition + Projectile.Size / 2 , null, Color.White, ro, texture.Size() / 2, Projectile.scale, spriteEffects, 0f);

            for (int A = 0; A < Projectile.DProj().Times[4] / 4; A++)
            {
                Main.spriteBatch.Draw(texture, Projectile.position - Main.screenPosition + Projectile.Size / 2 , null, new Color(55, 0, 0, 0), ro, texture.Size() / 2, Projectile.scale, spriteEffects, 0f);
            }
