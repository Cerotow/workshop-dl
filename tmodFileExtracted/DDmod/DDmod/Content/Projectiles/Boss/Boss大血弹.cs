...
{
    public class Boss大血弹 : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            if (Main.netMode != 1)
            {
                if (Projectile.ai[1] == 0)
                {
                    for (int i = 0; i < 5; i++)
                    {
                        Vector2 projDirection = Utils.RotatedBy(-Projectile.velocity.PerfectNormalize(), Main.rand.NextFloat(MathHelper.TwoPi), default) * Main.rand.NextFloat(0, 8.2F);
                        Projectile proj = Main.projectile[NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, projDirection, ModContent.ProjectileType<Boss血弹>(), Projectile.damage / 2, 0, -1, 0, 0, Main.rand.NextFloat(0.6F, 1.5F))];
                        proj.alpha = 5;
                    }
                }
                else
