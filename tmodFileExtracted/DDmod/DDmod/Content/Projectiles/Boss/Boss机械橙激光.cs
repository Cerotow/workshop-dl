...
{
    public class Boss机械橙激光 : ModProjectile
    {
        ...
        {
            NewDustChange4(30, Projectile.Center, Vector2.Zero, ModContent.DustType<光球粒子>(), 0, 4, true, 0.4F, 1.2F, 100, 0, new Color(255, 157, 100, 0), 2);
        }
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(texture, Projectile.Center - Projectile.velocity.PerfectNormalize() * 4 * Projectile.scale - Main.screenPosition, rectangle, color, Projectile.rotation + MathHelper.PiOver2, vector, new Vector2(Projectile.scale, Projectile.velocity.Length() / 2 * Projectile.localAI[0]), spriteEffects, 0f);

            vector = new Vector2(texture.Width / 2, 0);
            rectangle = new Rectangle(0, 16, texture.Width, 14);
            Main.spriteBatch.Draw(texture, Projectile.Center - (Projectile.velocity.PerfectNormalize() * 4 * Projectile.scale) - Projectile.velocity * Projectile.localAI[0] - Main.screenPosition, rectangle, color, Projectile.rotation + MathHelper.PiOver2, vector, Projectile.scale, spriteEffects, 0f);
