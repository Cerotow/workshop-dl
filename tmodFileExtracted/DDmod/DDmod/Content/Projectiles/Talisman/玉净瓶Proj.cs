...
{
    public class 玉净瓶Proj : Talismans
    {
        ...
        public override void PreUse()
        {
            ...
            Dust dust = Main.dust[NewDust(Projectile.Center - new Vector2(4) - new Vector2(0, 16).RotatedBy(Projectile.rotation), 6, 1, ModContent.DustType<光球粒子>(), 0, 0, 0, new Color(0,100,155,0))];
            ...
            if (Projectile.DProj().Times[0] % 60 == 0)
            {
                ...
                if (player.whoAmI == Main.myPlayer)
                    player.Heal(r);
                player.statMana += Crit ? Projectile.damage * 8 : Projectile.damage * 4;

                CombatText.NewText(new Rectangle((int)Projectile.position.X, (int)Projectile.position.Y, Projectile.width, Projectile.height), CombatText.HealMana, Crit ? Projectile.damage * 8 : Projectile.damage * 4);
            ...
            {
                NewDustChange(60, Projectile.Center, Vector2.Zero, ModContent.DustType<光球粒子>(), 0.1F, 2, Scale: Main.rand.NextFloat(0.6F, 1F),color: new Color(0, 100, 155, 0));
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (Projectile.DProj().Bool[0])
            {
                ...
                Main.spriteBatch.Draw(texture, Projectile.position + vector - Main.screenPosition, null, new Color(0,100,155,0), Projectile.rotation, texture.Size() / 2, 1, spriteEffects, 0f);
                Main.spriteBatch.Draw(texture, Projectile.position + vector - Main.screenPosition, null, new Color(0,100,155,0), Projectile.rotation, texture.Size() / 2, 1, spriteEffects, 0f);
            }
                return false;
        }
