...
namespace DDmod.Content.NPCs.Boss.狱火蛇
{
    ...
    public class 狱火蛇身 : ModNPC
    {
        ...
        public override void Load()
        {
            if (!Main.dedServ)
            {
                ...
                texture2 = ModContent.Request<Texture2D>(Texture + "3");
                texture2_Glow = ModContent.Request<Texture2D>(Texture + "3_Glow");
            }
        }
        ...
        public override void SetDefaults()
        {
            ...
            NPC.netAlways = true;
            NPC.scale = 1.5f;
            NPC.Dnpc().PenetrationProtection = 0.1F;
            NPC.Dnpc().MaxPenetrationProtection = 0.5F;
        ...
        public override void AI()
        {
            ...
            if (NPC.Dnpc().Stage == 4)
            {
                ...
                if (!NPC.Dnpc().Bool[4] && Main.npc[(int)NPC.ai[3]].ai[0] >= 100)
                {
                    ...
                    {
                        int GoreType = Mod.Find<ModGore>("狱火蛇3").Type;
                        int GoreType2 = Mod.Find<ModGore>("狱火蛇4").Type;
            ...
            //保持距离
            if (NPC.ai[1] < Main.npc.Length && HostNPC.active)
            {
                ...
                float Distance = (vector.Length() - (30 * NPC.scale)) / vector.Length();
                if (HostNPC.type == ModContent.NPCType<狱火蛇头>())
                {
                    Distance = (vector.Length() - (46 * NPC.scale)) / vector.Length();
                }
                NPC.velocity = Vector2.Zero;
                NPC.position = NPC.position + vector * Distance;
            }
            if (!HostNPC.active || (HostNPC.type != ModContent.NPCType<狱火蛇头>() && HostNPC.type != ModContent.NPCType<狱火蛇身2>() && HostNPC.type != NPC.type))
            {
                NPC.life = 0;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            texture = ModContent.Request<Texture2D>("DDmod/Content/NPCs/Boss/狱火蛇/狱火蛇身3").Value;
    ...
    public class 狱火蛇身2 : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (NPC.Dnpc().Stage == 4)
            {
                ...
                if (!NPC.Dnpc().Bool[4] && Main.npc[(int)NPC.ai[3]].ai[0] >= 100)
                {
                    if (Main.netMode != NetmodeID.Server)
                    {
                        int GoreType = Mod.Find<ModGore>("狱火蛇5").Type;
                        int GoreType2 = Mod.Find<ModGore>("狱火蛇6").Type;
            ...
            if (!HostNPC.active || (HostNPC.type != ModContent.NPCType<狱火蛇头>() && HostNPC.type != ModContent.NPCType<狱火蛇身>() && HostNPC.type != NPC.type))
            {
                NPC.life = 0;
                NPC.HitEffect(0, 10.0);
                NPC.active = false;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            Texture2D texture = TextureAssets.Npc[Type].Value;
            NPC HostNPC = Main.npc[(int)NPC.ai[3]];
            Texture2D textureGlow = 狱火蛇身.texture2_Glow.Value;
            if (NPC.Dnpc().Stage == 4)
            {
                texture = 狱火蛇身.texture2.Value;
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            if(NPC.Dnpc().Stage != 4)
            spriteBatch.Draw(Glow.Value, NPC.Center - screenPos, null, NPC.GetAlpha(new Color(255, 155, 155, 0)), NPC.rotation+ MathHelper.PiOver2, new Vector2(Glow.Width() / 2, Glow.Height() / 2), NPC.scale / 4, 0, 0f);
            //spriteBatch.Draw(Glow.Value, NPC.Center - screenPos, null, NPC.GetAlpha(new Color(0, 76, 255, 0)), NPC.rotation+ MathHelper.PiOver2, new Vector2(Glow.Width() / 2, Glow.Height() / 2), NPC.scale / 4, 0, 0f);
            //spriteBatch.Draw(Glow.Value, NPC.Center - screenPos, null, NPC.GetAlpha(new Color(0, 76, 255, 0)), NPC.rotation, new Vector2(Glow.Width() / 2, Glow.Height() / 2), NPC.scale / 4, 0, 0f);
            //spriteBatch.Draw(Glow.Value, NPC.Center - screenPos, null, NPC.GetAlpha(new Color(0, 76, 255, 0)), NPC.rotation, new Vector2(Glow.Width() / 2, Glow.Height() / 2), NPC.scale / 4, 0, 0f);
        }
    }
