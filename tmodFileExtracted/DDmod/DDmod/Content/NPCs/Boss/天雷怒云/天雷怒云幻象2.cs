...
namespace DDmod.Content.NPCs.Boss.天雷怒云
{
    ...
    public class 天雷怒云幻象2 : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (NPC.ai[1] >= 120 && NPC.ai[1]<=240 && NPC.ai[1] % 4 == 0&& NPC.ai[0]<2)
            {
            ...
            if(NPC.ai[0]==2)
            {
                npc = Main.npc[FindFirstNPC(ModContent.NPCType<天雷怒云幻象>())];
                if (npc.type == ModContent.NPCType<天雷怒云幻象>()&& NPC.ai[1] %60 == 0)
                {
                    Vector2 vector2 = npc.Center + new Vector2(Main.rand.Next(-npc.width / 2 * 10, npc.width / 2 * 10) / 10, 0);
                    NPC.NewNPCProj(NPC.Center + new Vector2(Main.rand.Next(-NPC.width / 2 * 10, NPC.width / 2 * 10) / 10, 0), new Vector2(0, 12), ModContent.ProjectileType<Boss电珠>(), 30, 1, -1, vector2.X, vector2.Y,Main.rand.NextFloat(0.5F,1.5F));
                }
