...
namespace DDmod.Content.NPCs.Boss.天地守卫
{
    ...
    public class 大地守卫替 : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            Main.LocalPlayer.Dplayer().Bossperspective(P.Center - new Vector2(0, 150), 10, false, 0.1F);
            ...
            else if (NPC.localAI[0] <= 3)
            {
                ...
                Text = Language.GetTextValue("Mods.DDmod.NPCDialogue.LifeGuard.ArousalDialogue"+ NPC.localAI[0]);
                if (TLTime >= 300)
                {
                    Text = "";
                }
            ...
            else if (NPC.localAI[0] == 10)
            {
                ...
                Text = Language.GetTextValue("Mods.DDmod.NPCDialogue.LifeGuard.ArousalDialogue" + 9);
            }
            else if (NPC.localAI[0] == 9)
            {
                if (!NPC.Dnpc().Bool[0])
                {
                    for (int r = 0; r < 5; r++)
                    {
                        for (int R = 0; R < 4; R++)
                        {
                            int a = NewDust(NPC.Center - new Vector2(4), 1, 1, ModContent.DustType<光圈粒子>(), 0, 0, -100, new Color(255, 10, 10, 40), 0.1F + 0.3F);
                            Main.dust[a].customData = new Vector3(1, 20, 4 * R);
                            Main.dust[a].velocity = Vector2.Zero;
                            Main.dust[a].alpha = 0;
                        }
                    }
                    NewDustChange3(100, NPC.Center - new Vector2(4), Vector2.Zero, ModContent.DustType<速度粒子>(), 8, 30, true, 3, 8, 100, 1000, new Color(255, 50, 50, 40));

                    NPC.Dnpc().Bool[0] = true;
                    SetEventFlagCleared(ref NPCDowned.星心对话, -1);
                    ...
                    DNPC.NewNPCs(NPC.GetSource_FromAI(), NPC.Center + new Vector2(0, NPC.height / 2), ModContent.NPCType<大地守卫>(), 0);
                }
                else
                {
                    NPC.active = false;
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            //绘制能量体大小
            spriteBatch.Draw(Glow.Value, NPC.position + new Vector2(NPC.width, NPC.height) / 2 - Main.screenPosition, null, new Color(255 - NPC.alpha, 50 - NPC.alpha, 50 - NPC.alpha, 0) * 1f, NPC.rotation, Glow.Size() / 2, NPC.scale * 1.15f, spriteEffects, 0f);
            spriteBatch.Draw(Glow.Value, NPC.position + new Vector2(NPC.width, NPC.height) / 2 - Main.screenPosition, null, new Color(0, 105 - NPC.alpha, 105 - NPC.alpha, 0) * 1f, NPC.rotation, Glow.Size() / 2, NPC.scale * 1.15f/ 2, spriteEffects, 0f);

            //如果没被击败就绘制贴图
            if (NPC.localAI[1] != 2) spriteBatch.Draw(texture, NPC.position + new Vector2(NPC.width, NPC.height) / 2 - Main.screenPosition, new Rectangle?(NPC.life <= NPC.lifeMax / 2 ? new Rectangle(texture.Width / 2, NPC.frame.Y, texture.Width / 2, texture.Height / 5) : new Rectangle(0, NPC.frame.Y, texture.Width / 2, texture.Height / 5)), Color.White, NPC.rotation, vector, NPC.scale, spriteEffects, 0f);
        }
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            /*
            Texture2D TextBubblesMouth = ModContent.Request<Texture2D>("DDmod/Textures/TextBubbles/大地守卫气泡口").Value;
            Texture2D TextBubbles = ModContent.Request<Texture2D>("DDmod/Textures/TextBubbles/大地守卫气泡").Value;
            if (Text != null && Text != "")
            {
                DDHelper.DrawTextBubbles(spriteBatch, TextBubblesMouth, TextBubbles,
                    NPC.Center, Text, 1, Color.White, Color.White,FontAssets.MouseText.Value, 300,0);
            }*/
            return false;
        }
