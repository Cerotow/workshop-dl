...
namespace DDmod.Content.NPCs.EliteMonster
{
    ...
    public class 丛林暴食怪 : ModNPC
	{
        ...
        public override void AI()
        {
            ...
            if (NPC.Dnpc().Stage == 0||NPC.AnyNPCs(ModContent.NPCType<小丛林暴食怪>()))
            {
                ...
                {
                    NPC.velocity = Distance.PerfectNormalize() * 2;
                }
                if (Distance.Y > 140)
                {
            ...
            else if (NPC.Dnpc().Stage == 1)
            {
                ...
                if (NPC.ai[1] < 300)
                {
                    if(NPC.ai[1]<120)
                    {
                        ...
                        if (Distance.Length() > 80)
                        {
                            ...
                            if (NPC.velocity.Length() > 10)
                                NPC.velocity *= 0.92F;
                            if (DDHelper.SpecifyDirection(Distance.ToRotation() + MathHelper.Pi, vector.ToRotation(), 0.3F))
                            {
                                NPC.ai[1]++;
                            }
                        }
                        else
                        {
                            ...
                            NPC.velocity += vector.PerfectNormalize() * 0.3f;
                            NPC.velocity *= 0.9F;
                        }
                    }
                    if(NPC.ai[1]==120)
                    ...
                    if (NPC.ai[1] > 120)
                    {
                        ...
                        else if((NPC.ai[1]>120 && (player.Center - NPC.Center).Length() < 80) || NPC.Dnpc().Bool[3])
                        {
                            NPC.Dnpc().Bool[3] = true;
                            NPC.velocity *= 0.9F;
                        }
                    }
                    if(NPC.velocity.Length()<1&& NPC.ai[1] > 180)
                    {
                        NPC.ai[1] = 300;
                    }
                ...
                else if (NPC.ai[1] < 900)
                {
                    ...
                    if (NPC.ai[1] >= 720)
                    {
                        ...
                        {
                            for(int a=0;a<3;a++)
                            NPC.NewNPCProj(NPC.Center, -Distance.PerfectNormalize().RotatedBy(Main.rand.NextFloat(-0.2F,0.2F)) * Main.rand.NextFloat(2, 5), ModContent.ProjectileType<Boss叶箭>(), 15, 1);
                        }
                        if (NPC.ai[1] % 40 == 0)
                        {
                            NPC.ai[2] = 12;
            ...
            if (DDHelper.SpecifyDirection(Distance.ToRotation() + MathHelper.Pi, (Main.player[NPC.target].Center - NPC.Center).ToRotation(), 0.3F))
            {
                if ((Main.player[NPC.target].Center - NPC.Center).Length() < 80&& NPC.ai[2]<-4)
                {
                    //药盒
                    NPC.ai[2] = 12;
                }
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
            ...
            [
                ...
                new FlavorTextBestiaryInfoElement(Language.GetTextValue("Mods.DDmod.NPCBestiary.丛林暴食怪"))
            ]);
		}
		public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            npcLoot.Add(new DropLocalPerClient(ModContent.ItemType<荆棘戒指>()));
            npcLoot.Add(ItemDropRule.MasterModeCommonDrop(ModContent.ItemType<丛林暴食怪圣物>()));
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<丛林暴食怪纪念章物品>(), 10));
        }
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            Texture2D texture = TextureAssets.Npc[NPC.type].Value;
            Asset<Texture2D> L = 丛林暴食怪根.L;
            Asset<Texture2D> L2 = 丛林暴食怪根.L2;
            NPC npc = Main.npc[(int)NPC.ai[0]];
            ...
            if (npc.active)
            {
                ...
                for (int W = 0; W < (npc.Center - NPC.Center).Length() / L.Height(); W++)
                {
                    if (W < (npc.Center - NPC.Center).Length() / L.Height() - 1)
                    {
                        ...
                        Main.spriteBatch.Draw(L2.Value, NPC.position + (npc.Center - NPC.Center).PerfectNormalize() * (8 + L.Height() * W-4) + vector - Main.screenPosition, new Rectangle?(new Rectangle(0, 0, (int)(L2.Width()), L2.Height())), color, (npc.Center - NPC.Center).ToRotation() - MathHelper.PiOver2, new Vector2(L2.Width() / 2, 16), 1, spriteEffects, 0f);
                    }
                    else
                    {
                        Vector2 P = NPC.position + (npc.Center - NPC.Center).PerfectNormalize() * (8 + L.Height() * W) + vector;
                        ...
                        Main.spriteBatch.Draw(L2.Value, NPC.position + (npc.Center - NPC.Center).PerfectNormalize() * (8 + L.Height() * W-4) + vector - Main.screenPosition, new Rectangle?(new Rectangle(0, 0, (int)(L2.Width()), (int)(L2.Height() - (L2.Height() - (npc.Center - NPC.Center).Length() % L2.Height())))), color, (npc.Center - NPC.Center).ToRotation() - MathHelper.PiOver2, new Vector2(L2.Width() / 2, 16), 1, spriteEffects, 0f);
                    }
                }
            }
        ...
        public override void FindFrame(int frameHeight)
        {
            ...
            {
                if ((player.Center - NPC.Center).Length() < 80)
                {
        ...
        public override void HitEffect(HitInfo hit)
		{
			...
			if (NPC.life <= 0)
			{
                ...
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    Asset<Texture2D> L = 丛林暴食怪根.L;
                    Asset<Texture2D> L2 = 丛林暴食怪根.L2;
                    int GoreType = Mod.Find<ModGore>("丛林暴食怪身").Type;
                    int GoreType2 = Mod.Find<ModGore>("丛林暴食怪身2").Type;
                    int GoreType3 = Mod.Find<ModGore>("丛林暴食怪1").Type;
                    int GoreType4 = Mod.Find<ModGore>("丛林暴食怪2").Type;
                    int GoreType5 = Mod.Find<ModGore>("丛林暴食怪3").Type;
                    ...
                    for (int W = 0; W < (npc.Center - NPC.Center).Length() / L.Height(); W++)
                    {
                        if (W < (npc.Center - NPC.Center).Length() / L.Height() - 1)
                        {
                            ...
                            P = NPC.position + (npc.Center - NPC.Center).PerfectNormalize() * (8 + L.Height() * W - 4) + vector;
