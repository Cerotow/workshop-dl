...
{
    public class StraGuardRelicTile : Relic
    {
        public override RelicType RelicType => RelicType.PreHardmode;
        public override string RelicTextureName2 => "DDmod/Content/Tiles/Relic/StraGuardRelicTile2";
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            for (int T = 0; T < eyeCount; T++)
            {
                if (Main.tile[i, j].TileFrameX == 0 && (Main.tile[i, j].TileFrameY == 0 || Main.tile[i, j].TileFrameY == 72))
                {
                    ...
                    Main.EntitySpriteDraw(texture2, drawPos, null, color, ro, texture2.Size() / 2 + new Vector2(0, 3), 0.8f, SpriteEffects.None, 0);
                }
            }
        }
    }
