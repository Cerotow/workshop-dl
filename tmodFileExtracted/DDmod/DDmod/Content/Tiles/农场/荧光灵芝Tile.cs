...
namespace DDmod.Content.Tiles.农场
{
    public class 荧光灵芝Tile : 作物
    {
        ...
        public override IEnumerable<Item> GetItemDrops(int i, int j)
        {
            ...
            if (A == 0)
            {
                yield return new Item(ModContent.ItemType<奇怪的发光孢子>());
            }
            if (A == 2)
            {
                yield return new Item(183,Main.rand.Next(2,6));
            }
            if (A == 3)
            {
                yield return new Item(ModContent.ItemType<荧光灵芝>(), Main.rand.Next(1, 3));
                yield return new Item(183, Main.rand.Next(5, 11));
            }
        }
