    ...
    public class 腐败果树 : 作物
    {
        ...
        public override IEnumerable<Item> GetItemDrops(int i, int j)
        {
            ...
            if (A == Max)
            {
                ...
                yield return new Item(4284, 5 - r);
                yield return new Item(4289, r);
            ...
            if (A > 0)
            {
                ...
                yield return new Item(619, Main.rand.Next(3, 8) * A);
            }
            yield return new Item(ModContent.ItemType<果树种子>());
        ...
        public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
        {
            ...
            if (!fail && Main.netMode != 2 && left == i && top == j)
            {
                if (A == 5 || A == 4)
                {
                    for (int L = 0; L < 4; L++)
                    {
                        for (int T = 0; T < 3; T++)
                        {
                            for (int a = 0; a < 4; a++)
                            {
                                Gore.NewGore(new EntitySource_TileEntity(tepowerCellFactory), new Vector2((float)left + L, (float)top + T) * 16f, Vector2.Zero, 915);
                            }
                        }
                    }
                }
                ...
                else
                {
                    for (int a = 0; a < 4; a++)
                    {
                        Gore.NewGore(new EntitySource_TileEntity(tepowerCellFactory), new Vector2((float)left + 2, (float)top + 3) * 16f, Vector2.Zero, 915);
                    }
                }
            }
        }
        ...
        public override bool CreateDust(int i, int j, ref int type)
        {
            for (int a = 0; a < 5; a++)
            {
                Dust.NewDust(new Vector2((float)i, (float)j) * 16f, 16, 16, 77, 0f, 0f, 1);
            }
            return false;
        }
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            {
                int r = 5 - Main.rand.Next(2, 4);
                Main.LocalPlayer.QuickSpawnItem(new EntitySource_TileEntity(tepowerCellFactory), 4284, 5 - r);
                Main.LocalPlayer.QuickSpawnItem(new EntitySource_TileEntity(tepowerCellFactory), 4289, r);
