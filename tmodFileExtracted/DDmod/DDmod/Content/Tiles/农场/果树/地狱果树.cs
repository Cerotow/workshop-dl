...
namespace DDmod.Content.Tiles.农场.果树
{
    public class 地狱果树 : 作物
    {
        ...
        {
            Glow = ModContent.Request<Texture2D>("DDmod/Content/Tiles/农场/果树/地狱果树_Glow");
        }
        ...
        public override IEnumerable<Item> GetItemDrops(int i, int j)
        {
            ...
            if (A == Max)
            {
                ...
                yield return new Item(5277,5-r);
                yield return new Item(5278,r);
            ...
            if (A > 0)
            {
                ...
                yield return new Item(5217, Main.rand.Next(3, 8) * A);
            }
            yield return new Item(ModContent.ItemType<果树种子>());
        ...
        public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
        {
            ...
            if (!fail && Main.netMode != 2 && left == i && top == j)
            {
                if (A == 5 || A == 4)
                {
                    for (int L = 0; L < 4; L++)
                    {
                        for (int T = 0; T < 3; T++)
                        {
                            for (int a = 0; a < 4; a++)
                            {
                                Dust.NewDust(new Vector2((float)left + L, (float)top + T) * 16f, 16, 16, 6, 0f, 0f, 1);
                            }
                        }
                    }
                }
                ...
                else
                {
                    for (int a = 0; a < 4; a++)
                    {
                        Dust.NewDust(new Vector2((float)left + 2, (float)top + 3) * 16f, 16, 16, 6, 0f, 0f, 1);
                    }
                }
            }
        }
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            {
                int r = 5 - Main.rand.Next(2, 4);
                Main.LocalPlayer.QuickSpawnItem(new EntitySource_TileEntity(tepowerCellFactory), 5277, 5 - r);
                Main.LocalPlayer.QuickSpawnItem(new EntitySource_TileEntity(tepowerCellFactory), 5278, r);
