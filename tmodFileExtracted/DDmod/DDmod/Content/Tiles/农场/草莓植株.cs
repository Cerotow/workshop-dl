...
namespace DDmod.Content.Tiles.农场
{
    public class 草莓植株 : 作物
    {
        ...
        public override IEnumerable<Item> GetItemDrops(int i, int j)
        {
            ...
            if (A == 0)
            {
                yield return new Item(ModContent.ItemType<草莓种子>());
            }
            if (A == Max)
            {
                yield return new Item(ModContent.ItemType<草莓>(),4);
            }
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (A == 3)
            {
                Main.LocalPlayer.QuickSpawnItem(new EntitySource_TileEntity(tepowerCellFactory), ModContent.ItemType<草莓>(), 4);
                tepowerCellFactory.Time = Time;
