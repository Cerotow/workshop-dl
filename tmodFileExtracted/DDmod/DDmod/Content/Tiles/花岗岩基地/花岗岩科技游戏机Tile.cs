...
{
	public class 花岗岩科技游戏机Tile : ModTile
	{
		...
		public override bool RightClick(int i, int j) {
			...
			*/
            ...
            int left = i - t.TileFrameX % (18 * 2) / 18;
            ...
            player.GetModPlayer<PlaystationPlayer>().UIStyle = 0;
		...
		public override void MouseOver(int i, int j) {
			...
			player.cursorItemIconID = ModContent.ItemType<花岗岩游戏机>();
        ...
        public string T => "DDmod/Content/Tiles/花岗岩基地/花岗岩科技游戏机Tile_Glow";
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			int left = i - tile.TileFrameX % (18 * 2) / 18;
			int top = j - tile.TileFrameY % (18 * 3) / 18;
			for (int A = 0; A < 255; A++)
			{
				if (player.active && player.GetModPlayer<PlaystationPlayer>().point == new Point16(left, top))
				{
					Main.spriteBatch.Draw(TileTexture.Value, new Vector2((float)(i * 16 - (int)Main.screenPosition.X), (float)(j * 16 - (int)Main.screenPosition.Y)) + zero, new Rectangle?(new Rectangle((int)tile.TileFrameX, (int)tile.TileFrameY + 58, 16, height)), Color.White, 0f, Vector2.Zero, 1f, 0, 0f);
				}
			}
		}
    }
