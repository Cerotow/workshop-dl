...
{
    public class 绿岩胶化舱Tile : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            DDSystem.Instance.DDEquipGlow.TryGetValue("绿岩胶化舱", out int GG);
        ...
        public override void HitWire(int i, int j)
        {
            ...
            if (DNPC.TileCountNPCS(ModContent.NPCType<绿岩史莱姆>(), tepowerCellFactory.Position) < 8&& NPCDowned.绿岩刷怪)
            {
                //到达关键帧需要同步
                ...
                if (tepowerCellFactory.Time % SpawnTime == 0)
                {
                    if (Main.netMode != 1)
                    {
                        int A = NewNPCs(new EntitySource_TileEntity(tepowerCellFactory), new Vector2(i + 3, j + 2) * 16, ModContent.NPCType<绿岩史莱姆>(), 1, 0, 0, 255);
                        Main.npc[A].velocity.X = (Main.rand.NextFloat(-2, 2));
        ...
        public override void EmitParticles(int i, int j, Tile tile, short tileFrameX, short tileFrameY, Color tileLight, bool visible)
        {
            ...
            if (Main.netMode != 2 && tepowerCellFactory.Dust)
            {
                for (int A = 0; A < 50; A++)
                {
                    NewDust(new Vector2(i + 2, j + 2) * 16 - new Vector2(4), 32, 5, DustType, Main.rand.NextFloat(-3, 3), -Main.rand.NextFloat(4, 8), Scale: Main.rand.NextFloat(0.5F, 1.5F));
                }
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            if (tepowerCellFactory != null)
            {
                ...
                if (NPCDowned.绿岩刷怪 && (tepowerCellFactory.Initiate || tepowerCellFactory.SpawnRunning))
                {
                    ...
                    {
                        spriteBatch.Draw(texture, new Vector2((float)(i * 16 - (int)Main.screenPosition.X), (float)(j * 16 - (int)Main.screenPosition.Y)) + zero, new Rectangle(tile.TileFrameX+108, tile.TileFrameY, 16, height), Lighting.GetColor(new Point(i, j)), 0f, Vector2.Zero, 1f, 0, 0f);
                        spriteBatch.Draw(Glow, new Vector2((float)(i * 16 - (int)Main.screenPosition.X), (float)(j * 16 - (int)Main.screenPosition.Y)) + zero, new Rectangle(tile.TileFrameX+108, tile.TileFrameY, 16, height), Color.White, 0f, Vector2.Zero, 1f, 0, 0f);
                    }
                }
                else
                {
