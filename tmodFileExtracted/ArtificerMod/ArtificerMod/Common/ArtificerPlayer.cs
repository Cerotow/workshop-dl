...
namespace ArtificerMod.Common
{
    public class ArtificerPlayer : ModPlayer
    {
        ...
        public bool callCoolBonus; // Can be set to true via. a mod.Call to prevent the no artificer set bonus penalty
        private bool callCoolFlag;
        ...
        public bool clockworkAutoAbility;
        ...
        public bool hideMagicWeps;
        public float bonusFlight = 0f;
        public bool fastFall = false;
        public Item effigyEquip;
        public Item relicEquip;
        public int relicRegen = 0;
        public Item gamblecoreEquip;
        ...
        public bool accFlagAs; // Nova Armor
        ...
        public bool cFlagAs; // Nova Emblem
        ...
        public override void ResetEffects()
        {
            ...
            noArtificerBonuses = !(ornateSetBonus || starplateSetBonus || techSetBonus
                                    ...
                                    || astralSetBonus || callCoolBonus || callCoolFlag);
        ...
        public void UpdateAbilityCooldown()
        {
            if (Player.whoAmI == Main.myPlayer)
            {
                ...
                if (cooldownIndex != -1)
                {
                    ...
                    // Accesories (Energy Cell, Artificer Emblem, Cooling Cell, Nova Emblem):
                    ...
                    // Mod.Call additions
                    for (int i = 0; i < CallIntegration.AddOnCooldownMods.Count; i++)
                    {
                        CooldownModData cooldownData = CallIntegration.AddOnCooldownMods[i];
                        if(cooldownData.ActivityChanged(Player))
                        {
                            newCooldown = !cooldownData.ShouldApply(Player) ? (newCooldown / cooldownData.CooldownMult) : (newCooldown * cooldownData.CooldownMult);
                        }
                    }
        ...
        public void AddCooldown(int secondsOfCooldown, int ticksOfCooldown = -1)
        {
            // convert seconds to ticks
            int duration = ticksOfCooldown > 0 ? ticksOfCooldown : secondsOfCooldown * 60;

            // Nova set bonus
            bool novaBoost = false;
            ...
            if (astralSetBonus)
            {
                ...
                // Also triggers Nova Surge effect of the Nova set bonus
                novaBoost = true;
            }
            if (energyCellBonus)
            {
            ...
            {
                CooldownModData cooldownData = CallIntegration.AddOnCooldownMods[i];
                if (cooldownData.ShouldApply(Player))
                {
                    duration = (int)(duration * cooldownData.CooldownMult);
                }
            }
            ...
            if (novaBoost)
            {
                // Adds Nova Surge for the same duration of the cooldown, or up to 90 seconds if longer than that
                Player.AddBuff(ModContent.BuffType<NovaSurge>(), Math.Min(duration, 5400));
            }
        }
        ...
        public override void ModifyHurt(ref Player.HurtModifiers modifiers)
        {
            ...
            if(Player.HasBuff<SwaggerCapeBuff>())
            {
        ...
        public override bool PreKill(double damage, int hitDirection, bool pvp, ref bool playSound, ref bool genGore, ref PlayerDeathReason damageSource)
        {
            ...
            if(abilityAcc && starwriter 
                && Main.myPlayer == Player.whoAmI
                && !Player.HasBuff(ModContent.BuffType<AbilityCooldown>()) 
                && Player.ownedProjectileCounts[ModContent.ProjectileType<StarwriterSavePos>()] >= 1)
            {
                StarwriterActivate();
                AddCooldown(120);
                return false;
        ...
        public override void UpdateLifeRegen()
        {
            ...
            if(Player.HasBuff(ModContent.BuffType<PrimevalHarvest>()))
            {
                Player.lifeRegen += (int)Utils.Remap(relicRegen, 150, 3000, 1f, 10f);
            }
            else
            {
                relicRegen = 0;
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (hit.Crit == true) 
            {
                if (lifesurge && lifesurgeCooldown <= 0 && !target.SpawnedFromStatue && target.CanBeChasedBy())
                {
                    Player.AddBuff(ModContent.BuffType<LifesurgeBuff>(), Main.rand.Next(120, 180));
                    lifesurgeCooldown = 300;
                }
                if (heartsurge && heartsurgeCooldown <= 0 && !target.SpawnedFromStatue && target.CanBeChasedBy())
                {
            ...
            if(target.life <= 0 && !target.SpawnedFromStatue && target.CanBeChasedBy()) // Killing blow effects
            {
        ...
        private void ArmorLighting(int head, int body, int legs)
        {
            ...
            if (head == EquipLoader.GetEquipSlot(mod, "AstralHeadgearG", EquipType.Head))
            {
                helmR = 0.3f;
                helmG = 0.7f;
                helmB = 0.4f;
            ...
            if (body == EquipLoader.GetEquipSlot(mod, "AstralCuirassG", EquipType.Body))
            {
                helmR = 0.3f;
                helmG = 0.7f;
                helmB = 0.4f;
            ...
            if (legs == EquipLoader.GetEquipSlot(mod, "AstralGreavesG", EquipType.Legs))
            {
                helmR = 0.3f;
                helmG = 0.7f;
                helmB = 0.4f;
        ...
        {
            if (hideMagicWeps && Player.HeldItem != null && Player.HeldItem.damage > 0 && Player.HeldItem.DamageType == DamageClass.Magic)
            {
                drawInfo.ItemLocation = new Vector2(Main.rightWorld, Main.bottomWorld);
            }
        ...
        private void LihzahrdRetaliation()
        {
            if (lihzahrdSetBonus)
            {
                if (Player.ownedProjectileCounts[ModContent.ProjectileType<LihzahrdArmorBeam>()] < 3)
                {
                    ...
                    for (int i = 0; i < targets.Count; i++)
                    {
                        if (Main.myPlayer == Player.whoAmI)
                        {
                            ...
                            Projectile.NewProjectile(Player.GetSource_Misc("SetBonus_LihzahrdArmor"), Player.Center, projVelocity, 
                                ModContent.ProjectileType<LihzahrdArmorBeam>(),
                                (int)Player.GetTotalDamage(DamageClass.Generic).ApplyTo(beamDMG), 10f, Player.whoAmI);
                        }
                    }
                ...
                Player.AddBuff(ModContent.BuffType<SolarOverdrive>(), Main.rand.Next(250, 500));
            }
        }
        ...
        private void LowGravResist()
        {
            if (heavyShoes)
            {
                if(Player.gravity * Player.gravDir < Player.defaultGravity)
                {
                    Player.gravity = MathHelper.Lerp(Player.gravity, Player.defaultGravity * Player.gravDir, 0.4f);
                }
            }
        }
        ...
        {
            if(clockworkAutoAbility 
                && !Player.HasBuff(ModContent.BuffType<AbilityCooldown>()) && !abilityCooldown && !cooldownFlag
                && inCombatTimer > 0)
            {
        ...
        private void FlightTimeBoost()
        {
            ...
            if (Player.wings != -1)
            {
                Player.wingTimeMax += (int)(Player.GetWingStats(Player.wings).FlyTime * bonusFlight);
                if(Player.rocketBoots != 0 && Player.rocketTime != 0)
                {
                    Player.wingTimeMax += (int)(42f * bonusFlight);
                }
            }
        }
        ...
        {
            for (int i = 0; i < CallIntegration.AddOnAbilities.Count; i++)
            {
                ArtificerAbilityData abilityData = CallIntegration.AddOnAbilities[i];
                if (abilityData.CanActivate(Player))
                {
                    NovaVisual();
                    if(abilityData.UseAbility(Player))
                    {
                        AddCooldown(60, abilityData.CooldownTime);
                    }
                    break;
                }
            }
            ...
            else if (gamblecoreEquip != null)
            {
                SoundEngine.PlaySound(SoundID.CoinPickup, Player.position);
                if (Main.LocalPlayer == Player)
                {
                    int dmg = (int)Player.GetTotalDamage(DamageClass.Generic).ApplyTo(gamblecoreEquip.damage);
                    float kb = Player.GetTotalDamage(DamageClass.Generic).ApplyTo(gamblecoreEquip.knockBack);

                    float face = 2; // The face the dice will land on. Defaults to 2 (Silver) (52.10%)

                    if(Main.rand.NextBool(2000)) // Platinum (00.05%); x20 damage
                    {
                        face = 5;
                    }
                    else if (Main.rand.NextBool(100)) // Gold (00.99%); x5 damage
                    {
                        face = 4;
                    }
                    else if (Main.rand.NextBool(40)) // Random (02.47%)
                    {
                        face = 6;
                    }
                    else if (Main.rand.NextBool(10)) // None (09.65%); no coin projectiles/pickups, but dice explodes with doubled stats
                    {
                        face = 0;
                        dmg *= 2;
                        kb *= 2f;
                    }
                    else if (Main.rand.Next(10) < 4) // Copper (34.73%); x0.5 damage
                    {
                        face = 1;
                    }

                    Projectile.NewProjectile(Player.GetSource_Accessory(gamblecoreEquip), Player.Center + new Vector2(0, -80), 
                        Vector2.Zero, ModContent.ProjectileType<GamblecoreCube>(), dmg, kb, Player.whoAmI, face);
                }
                AddCooldown(20);
            }
            ...
            else if(effigyEquip != null)
            {
                SoundEngine.PlaySound(SoundID.Item103, Player.position);
                if(Main.LocalPlayer == Player)
                {
                    Projectile.NewProjectile(Player.GetSource_Accessory(effigyEquip), Player.Center, Vector2.Zero, ModContent.ProjectileType<SinisterAura>(), 0, 0, Player.whoAmI);
                }
                AddCooldown(40);
            }
            ...
            {
                Player.AddBuff(ModContent.BuffType<ChampionProtec>(), 90);
                AddCooldown(20);
            }
            else if (funiDungeonsArtifact)
            {
                MCDomesticBombingMoment(400);
                AddCooldown(50);
            }
            else if (relicEquip != null)
            {
                SoundEngine.PlaySound(SoundID.Item43, Player.position);
                if (Main.LocalPlayer == Player)
                {
                    Projectile.NewProjectile(Player.GetSource_Accessory(effigyEquip), Player.Center, Vector2.Zero, ModContent.ProjectileType<PrimevalAura>(), 0, 0, Player.whoAmI);
                }
                AddCooldown(40);
            }
            ...
            {
                Player.AddBuff(ModContent.BuffType<RetributionBuff>(), 1200);
                SetRetributionPow();
                AddCooldown(50);
            }
            else if (arcanum)
            ...
            {
                Player.AddBuff(ModContent.BuffType<CyborgCoreBuff>(), 600);
                AddCooldown(50);
            }
            else if (blaster2)
            {
        ...
        private void StimActivate(bool hp, int tier)
        {
            ...
            else
            {
                ...
                {
                    case 1:
                        Player.AddBuff(ModContent.BuffType<ManaStim1>(), 600);
                        break;
                    case 2:
                        Player.AddBuff(ModContent.BuffType<ManaStim2>(), 600);
                        break;
                    case 3:
                        Player.AddBuff(ModContent.BuffType<ManaStim3>(), 600);
                        break;
                    case 4:
                        Player.AddBuff(ModContent.BuffType<ManaStim4>(), 600);
                        break;
                    default:
                        break;
                }
        ...
        private void OrnateFlash(float maxDetectionDistance)
        {
            ...
            for (int j = 0; j < Main.maxPlayers; j++)
            {
                ...
                if (sqrDistanceToTarget < sqrMaxDetectDistance)
                {
                    ...
                    {
                        target.AddBuff(ModContent.BuffType<OrnateSpark>(), Main.rand.Next(300, 600));
                    }
                }
            }
            ...
            if (Player == Main.LocalPlayer && ornateEquip != null)
            {
                ...
                for (int i = 0; i < numberProjectiles; i++)
                {
                    ...
                    int proj = Projectile.NewProjectile(Player.GetSource_Accessory(ornateEquip), rotatedHandPosition, perturbedSpeed, ModContent.ProjectileType<OrnateRingFlash>(), 0, 0f, Player.whoAmI);
        ...
        private void RemoveDebuffs(int accType)
        {
            switch(accType)
            {
                case 4:
                    ...
                    Player.HasBuff(BuffID.Venom))
                    {
                        Player.AddBuff(ModContent.BuffType<Refresh>(), 600);
                    }

                    for(int i = 0; i < Main.maxPlayers; i++)
                    {
                        var plr = Main.player[i];
                        if(plr.active && !plr.dead && ((!Player.hostile && !plr.hostile) || Player.team == plr.team || plr == Player)
                            && Vector2.DistanceSquared(plr.Center, Player.Center) < 400f * 400f)
                        {
                            plr.ClearBuff(BuffID.Silenced);
                            plr.ClearBuff(BuffID.Slow);
                            plr.ClearBuff(BuffID.Confused);
                            plr.ClearBuff(BuffID.Poisoned);
                            plr.ClearBuff(BuffID.Bleeding);
                            plr.ClearBuff(BuffID.Stoned);
                            plr.ClearBuff(BuffID.Darkness);
                            plr.ClearBuff(BuffID.Cursed);
                            plr.ClearBuff(BuffID.BrokenArmor);
                            plr.ClearBuff(BuffID.Weak);
                            plr.ClearBuff(BuffID.Burning);
                            plr.ClearBuff(BuffID.Blackout);
                            plr.ClearBuff(BuffID.OgreSpit);
                            plr.ClearBuff(BuffID.WitheredArmor);
                            plr.ClearBuff(BuffID.Ichor);
                            plr.ClearBuff(BuffID.Rabies);
                            plr.ClearBuff(BuffID.Electrified);
                            plr.ClearBuff(BuffID.WitheredWeapon);
                            plr.ClearBuff(BuffID.Frozen);
                            plr.ClearBuff(BuffID.VortexDebuff);
                        }
                    }
                    ...
                    for (int i = 0; i < 300; i++)
                    {
                        Vector2 speed = Main.rand.NextVector2Circular(400f, 400f);
                        Dust d = Dust.NewDustPerfect(Player.Center + speed, 74, Vector2.Zero, Scale: 0.5f);
                        d.noGravity = true;
                    }
                    break;
                case 3:
                    ...
                    for (int i = 0; i < Main.maxPlayers; i++)
                    {
                        var plr = Main.player[i];
                        if (plr.active && !plr.dead && ((!Player.hostile && !plr.hostile) || Player.team == plr.team || plr == Player)
                            && Vector2.DistanceSquared(plr.Center, Player.Center) < 240f * 240f)
                        {
                            plr.ClearBuff(BuffID.OnFire);
                            plr.ClearBuff(BuffID.Frostburn);
                            plr.ClearBuff(BuffID.CursedInferno);
                            plr.ClearBuff(BuffID.ShadowFlame);
                            plr.ClearBuff(BuffID.Poisoned);
                            plr.ClearBuff(BuffID.Venom);
                        }
                    }
                    ...
                    for (int i = 0; i < 200; i++)
                    {
                        Vector2 speed = Main.rand.NextVector2Circular(240f, 240f);
                        Dust d = Dust.NewDustPerfect(Player.Center + speed, 74, Vector2.Zero, Scale: 0.5f);
                        d.noGravity = true;
                    }
                    break;
        ...
        private void Retribution()
        {
            ...
            {
                float dmgBoost = 0.1f + retributionPower;
                float critBoost = 10f + (retributionPower * 100f);
                if (dmgBoost > 0.15f)
                {
                    dmgBoost = 0.15f;
                }
        ...
        {
            retributionPower = retributionCharge * 0.0002f;
            if (retributionPower > 0.05f)
            {
                retributionPower = 0.05f;
            }
        ...
        private void PrismaticArcanum()
        {
                ...
                && Main.myPlayer == Player.whoAmI)
            {
                ...
                if (fCharmEquip != null)
                {
                    dmg = (int)Player.GetTotalDamage(DamageClass.Generic).ApplyTo(135);
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    Main.projectile[nightglow].tileCollide = false;
                }
                for (int i = 0; i < 10; i++)
                {
        ...
        private void SolarBlaster()
        {
                ...
                && Main.myPlayer == Player.whoAmI)
            {
                ...
                float rotation = MathHelper.ToRadians(7.5f);
                for (int i = 0; i < numberProjectiles; i++)
                {
        ...
        private void ShootMissiles()
        {
            if ((missileEquip != null || fCharmEquip != null) && missilesToShoot > 0 && missilesTimer <= 0 && Main.myPlayer == Player.whoAmI)
            {
                ...
                Vector2 velocity = new Vector2(0f, -16f * Player.gravDir);
        ...
        private void RepulseRocket()
        {
            if (Player == Main.LocalPlayer && rRocketEquip != null)
            {
                ...
                Vector2 velocity = SimpleProjVelocity(12.5f, Player.Center);
        ...
        private void FirstPrism()
        {
                ...
                && Player.ownedProjectileCounts[ModContent.ProjectileType<FirstPrismProj>()] < 1 && Main.myPlayer == Player.whoAmI)
            {
                ...
                if (fCharmEquip != null)
                {
                    dmg = (int)Player.GetTotalDamage(DamageClass.Generic).ApplyTo(155);
        ...
        private static bool BulwarkSpawnCheck(Vector2 aimPos)
        {
        ...
        private void BulwarkSummon(Vector2 spawnPos)
        {
                ...
                && Player.ownedProjectileCounts[ModContent.ProjectileType<FirstPrismProj>()] < 1 && Main.myPlayer == Player.whoAmI)
            {
                ...
                if (fCharmEquip != null)
                {
                    dmg = (int)Player.GetTotalDamage(DamageClass.Generic).ApplyTo(80);
        ...
        private void MCDomesticBombingMoment(float maxRange)
        {
            if(Player == Main.LocalPlayer && dungeonsReference != null)
            {
                ...
                float statMult = 1f + (harvesterCharge / 100f);
                ...
                float velocityScalar = harvesterCharge / 250f;
                velocityScalar = Math.Clamp(velocityScalar, 0.95f, 1.75f);

                float scaleFactor = harvesterCharge / 350f;
                scaleFactor = Math.Clamp(scaleFactor, 0.5f, 1.3f);

                for (int i = 0; i < dustNum; i++)
                {
                    Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
                    Dust d = Dust.NewDustPerfect(Player.Center + speed * 30, DustID.DungeonSpirit, speed * 32f * velocityScalar, Scale: 1.4f * scaleFactor);
                ...
                {
                    Vector2 speed = Main.rand.NextVector2Circular(1f, 1f);
                    Dust d = Dust.NewDustPerfect(Player.Center + speed * 10, DustID.DungeonSpirit, speed * 32f * velocityScalar, Scale: 1.2f * scaleFactor);
                ...
                for (int i = 0; i < dustNum2; i++)
                {
                    ...
                    Dust d = Dust.NewDustPerfect(Player.Center + speed * 20, DustID.DungeonSpirit, speed * 21f * velocityScalar, Scale: 1f * scaleFactor);
                ...
                for (int i = 0; i < dustNum3; i++)
                {
                    ...
                    Dust d = Dust.NewDustPerfect(Player.Center + speed * 10, DustID.DungeonSpirit, speed * 12f * velocityScalar, Scale: 0.8f * scaleFactor);
        ...
        private void SpawnProbes()
        {
                ...
                && Player.ownedProjectileCounts[ModContent.ProjectileType<MiniProbe>()] < 1 && Main.myPlayer == Player.whoAmI)
            {
                ...
                if (fCharmEquip != null)
                {
                    dmg = (int)Player.GetTotalDamage(DamageClass.Generic).ApplyTo(70);
        ...
        private void ReaperDR()
        {
            ...
            {
                float frDR = 0.15f;
                int drBoost = Math.Clamp(reaperDRCounter, 0, 15);
                for(int i = 0; i < drBoost; i++)
                {
                    frDR += 0.02f;
                }
                frDR = Math.Clamp(frDR, 0.15f, 0.45f);
                Player.endurance = 1f - (frDR * (1f - Player.endurance));
            }
            else
            {
                reaperDRCounter = 0;
        ...
        private void LuckyPresentBuffs()
        {
            ...
            switch (presentLuck) 
            {
                ...
                case 1: // Last roll was good RNG
                    ...
                    break;
                default: // Neutral
                    presentLuck = (Main.rand.Next(100) < 35) ? -1 : 1;
                    break;
            }
            ...
            if(presentLuck < 0) 
            {
                buffTimeInc = (Main.rand.Next(100) < 30);
            }
            else 
            {
                buffTimeInc = (Main.rand.Next(100) < 65);
            }
        ...
        private void StellarResonator()
        {
            if (Player == Main.LocalPlayer && resonatorEquip != null && resonatorActive)
            {
                ...
                {
                    // Multiplicative 20% DR
                    Player.endurance = 1f - (0.2f * (1f - Player.endurance));
                }
            }
            else
            {
        ...
        private void LienKnives()
        {
            if (homageEquip != null && knifeWaves > 0 && knifeTimer <= 0 && Main.myPlayer == Player.whoAmI)
            {
                ...
                Vector2 velocity = SimpleProjVelocity(22f, Player.Center);
        ...
        {
            if (cooldownFlag != abilityCooldown && !Player.HasBuff(ModContent.BuffType<AbilityCooldown>()))
            {
                ...
                if (ModContent.GetInstance<ConfigClient>().cooldownAwareness)
                {
                    for(int i = 0; i < 15; i++)
                    {
                        int dustID;
                    ...
                    if(Player == Main.LocalPlayer)
                    {
                        CombatText.NewText(new Rectangle((int)Player.position.X, (int)Player.position.Y, Player.width, Player.height), Colors.RarityYellow, Language.GetTextValue("Mods.ArtificerMod.CommonItemtooltip.AbilitiesRefreshed"));
                    }
