...
namespace ArtificerMod.Common
{
    public class AbilityAccessories : GlobalItem
    {
        ...
        {
            return IsAbilityAccessory(entity); 
        }
        ...
        public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
        {
            ...
            float netCoolMods = GetCoolMods(localPlayer);
            ...
            if(!IsBaseModContent(item.type))
            {
                for (int i = 0; i < CallIntegration.AddOnAbilities.Count; i++)
                {
                    ArtificerAbilityData abilityData = CallIntegration.AddOnAbilities[i];
                    if (abilityData.AccessoryID == item.type)
                    {
                        string text = Language.GetTextValue("Mods.ArtificerMod.CommonItemtooltip.ForItems.ActivatedAbility");
                        text += abilityData.AbilityDescription;
                        tooltips.Insert(index, new TooltipLine(Mod, "abilityDesc", text));
                        break;
                    }
                }
            }
        ...
        private static float FetchCooldown(Item item)
        {
            int type = item.type;
            ...
            List<int> cooldown20 = new List<int>
            {
                ...
                ModContent.ItemType<GamblerFallacy>(),
            ...
            {
                ModContent.ItemType<SinisterEffigy>(),
                ModContent.ItemType<PrimevalRelic>(),
            ...
            else
            {
                ...
                {
                    ArtificerAbilityData abilityData = CallIntegration.AddOnAbilities[i];
                    if(abilityData.AccessoryID == type)
                    {
                        return abilityData.CooldownTime / 60f;
                    }
                }
            }
        ...
        private static float FetchCooldown(int itemType)
        {
            List<int> cooldown10 = new List<int>
            {
                ModContent.ItemType<AugmentedLeg>(),
            ...
            if (cooldown10.Contains(itemType))
            {
                return 10;
            }
            else if (cooldown15.Contains(itemType))
            {
                return 15;
            }
            else if (cooldown20.Contains(itemType))
            {
                return 20;
            }
            else if (cooldown25.Contains(itemType))
            {
                return 25;
            }
            else if (cooldown30.Contains(itemType))
            {
                return 30;
            }
            else if (cooldown40.Contains(itemType))
            {
                return 40;
            }
            else if (cooldown45.Contains(itemType))
            {
                return 45;
            }
            else if (cooldown50.Contains(itemType))
            {
                return 50;
            }
            else if (cooldown60.Contains(itemType))
            {
                return 60;
            }
            else if (cooldown90.Contains(itemType))
            {
                return 90;
            }
            else if (cooldown120.Contains(itemType))
            {
                return 120;
            }
            else
            {
                ...
                {
                    ArtificerAbilityData abilityData = CallIntegration.AddOnAbilities[i];
                    if (abilityData.AccessoryID == itemType)
                    {
                        return abilityData.CooldownTime / 60f;
                    }
                }
        ...
        public static float GetCoolMods(Player player)
        {
            if (!player.TryGetModPlayer(out ArtificerPlayer artificer))
            {
                return 1f;
            ...
            for (int i = 0; i < CallIntegration.AddOnCooldownMods.Count; i++)
            {
                ...
                {
                    netMod *= cooldownData.CooldownMult;
                }
            }
        ...
        private static string GetTooltipLineTarget(Item item)
        {
            int type = item.type;
            ...
            if (tooltips2.Contains(type))
            {
                return "Tooltip1";
            }
            else if (tooltips3.Contains(type))
            {
                return "Tooltip2";
            }
            ...
            else 
            {
                for (int i = 0; i < CallIntegration.AddOnAbilities.Count; i++)
                {
                    ...
                    if (abilityData.AccessoryID == type)
                    {
                        return abilityData.PlaceholderTooltipName;
                    }
                }
            }
        ...
        private static bool HasFlavorText(Item item)
        {
            int type = item.type;
        ...
        /// <summary>
        /// Returns if the item the given type is an ability accessory from Artificer's Accessories. Used to prevent checking add-on content for description text whenever possible.
        /// </summary>
        private static bool IsBaseModContent(int type)
        {
            List<int> baseModAbilityItems = new List<int>
            {
                ModContent.ItemType<GamblerFallacy>(),
                ModContent.ItemType<AugmentedLeg>(),
                ModContent.ItemType<SinisterEffigy>(),
            ...
            return baseModAbilityItems.Contains(type);
        }

        public static bool IsAbilityAccessory(Item item)
        {
            return FetchCooldown(item) != -1f; // This logic determines if the accessory has an ability
        }
        public static bool IsAbilityAccessory(int itemID)
        {
            return FetchCooldown(itemID) != -1f;
        }
    }

    public class CooldownAccessories : GlobalItem
    {
        public override bool AppliesToEntity(Item entity, bool lateInstantiation)
        {
            return CountsAsCooldownAcc(entity.type);
        }

        private bool CountsAsCooldownAcc(int itemType)
        {
            List<int> cooldownAccs = CallIntegration.AddOnCooldownAccs;
            cooldownAccs.Add(ModContent.ItemType<EnergyCell>());
            cooldownAccs.Add(ModContent.ItemType<ArtificerEmblem>());
            cooldownAccs.Add(ModContent.ItemType<CoolingCell>());
            cooldownAccs.Add(ModContent.ItemType<ClockworkStopwatch>());
            cooldownAccs.Add(ModContent.ItemType<NovaEmblem>());
            return cooldownAccs.Contains(itemType);
        }
        ...
        public override void UpdateAccessory(Item item, Player player, bool hideVisual)
        {
            if(player.TryGetModPlayer(out ArtificerPlayer artificer))
            {
                artificer.cooldownAcc = true;
            }
        }

        public override bool CanAccessoryBeEquippedWith(Item equippedItem, Item incomingItem, Player player)
        {
            bool equippedCoolAcc = CountsAsCooldownAcc(equippedItem.type);
            bool incomingCoolAcc = CountsAsCooldownAcc(incomingItem.type);
            if (equippedCoolAcc) 
            {
                return !incomingCoolAcc;
            }
            if (incomingCoolAcc)
            {
                return !equippedCoolAcc;
            }
            return true;
        }
    }
