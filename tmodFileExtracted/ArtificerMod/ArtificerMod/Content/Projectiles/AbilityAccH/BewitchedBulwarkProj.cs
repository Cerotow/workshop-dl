        ...
        public override void AI()
		{
			...
			if (Main.rand.NextBool(2))
			{
				for (int i = 0; i < 25; i++)
				{
					...
					Dust d = Dust.NewDustPerfect(initalPos + speed * 275, DustID.Shadowflame, speed * 0, Scale: 0.9f);
					d.noGravity = true;
				}
			}
			...
			if (attackCooldown <= 0)
			{
				...
				{
					Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
					Dust d = Dust.NewDustPerfect(initalPos + speed * 275, DustID.Shadowflame, speed * -10f, Scale: 0.9f);
					d.noGravity = true;
				}
			}

			float maxDetectDistance = 275;
