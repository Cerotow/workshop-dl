...
{
	public class LifelineHook : ModProjectile
	{
		private const string ChainTexturePath = "ArtificerMod/Content/Projectiles/AbilityAccH/LifelineChain"; // The folder path to the chain sprite
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			if (!HookRetracting)
			{
				...
				target.AddBuff(ModContent.BuffType<Leech2>(), 600);
				Projectile.damage = 0;
		...
		private void UpdateTethers(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++)
			{
				...
				if (i != Projectile.whoAmI
					...
					&& currentProjectile.ModProjectile is LifelineHook leechProjectile
		...
		public override void AI()
		{
			...
			else if (vectorToOwner.Length() > (IsStickingToTarget ? 2000f : 750f)) // Retract the hook if it's too far away
			{
				HookRetracting = true;
			}
			if(vectorToOwner.Length() > 2400)
            {
				farFromOwner = true;
			}
			...
			if (!getArtificer || (artificer.tetherLeech <= 0 && !artificer.fCharm))
			{
				HookRetracting = true;
			}
		...
		private void StickyAI()
		{
			...
			if (Projectile.localAI[0] > 620 || projTargetIndex < 0 || projTargetIndex >= 200)
			{
				HookRetracting = true;
				return;
			}
		...
		public void Lifesteal(NPC target)
		{
			if (Projectile.owner == Main.myPlayer)
			{
				...
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, Projectile.velocity * 0f, ModContent.ProjectileType<LifelineHeal>(), 0, 0f, Projectile.owner, healTargetID, 8f);
			}
		}
