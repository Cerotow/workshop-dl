...
{
	public class LeechingTetherHook : ModProjectile
	{
		private const string ChainTexturePath = "ArtificerMod/Content/Projectiles/AbilityAccPH/LeechingTetherChain"; // The folder path to the chain sprite
		...
		private void UpdateTethers(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++)
			{
				...
				if (i != Projectile.whoAmI
					...
					&& currentProjectile.ModProjectile is LeechingTetherHook leechProjectile
		...
		public override void AI()
		{
			...
			else if (vectorToOwner.Length() > (IsStickingToTarget ? 1500f : 500f)) // Retract the hook if it's too far away
			{
				HookRetracting = true;
			}
			if (vectorToOwner.Length() > 1600)
			{
				farFromOwner = true;
			}
		...
		private void StickyAI()
		{
			...
			// Every 30 ticks, perform a hit effect
		...
		public void Lifesteal(NPC target)
		{
			if (Projectile.owner == Main.myPlayer)
			{
				...
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, Projectile.velocity * 0f, ModContent.ProjectileType<LeechingTetherHeal>(), 0, 0f, Projectile.owner, healTargetID, 4f);
			}
		}
