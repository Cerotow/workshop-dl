...
namespace ArtificerMod.Content.Items.ArmorPH
{
	...
	public class TechHeadgear : ModItem
	{
		...
		{
			string bonusDesc = Language.GetTextValue("Mods.ArtificerMod.CommonItemtooltip.Armorset.Tech");

			if(CrossModHelper.AdjustClassStats("CaptureDiscClass", "CaptureDamage", player, atkSpd: 0.05f))
			{
                bonusDesc += "\n" + CrossModHelper.GetCrossmodText("CaptureClass.IncCaptureSpd", 5);
            }
            if (CrossModHelper.ThoriumBardBuffRange(5, player))
            {
                bonusDesc += "\n" + CrossModHelper.GetCrossmodText("Thorium.BardBuffRange", 5);
            }

            player.setBonus = bonusDesc; 
